...
from run_event_kernel import (
    ...
    build_run_event_kernel_artifact,
...
RUN_ARTIFACT_PATH = ROOT / POST_MVP_RUN_ARTIFACT_PATH
STEP_LIST_ARTIFACT_PATH = ROOT / POST_MVP_STEP_LIST_ARTIFACT_PATH
...
RUN_EVENT_KERNEL_ARTIFACT_PATH = ROOT / POST_MVP_RUN_EVENT_KERNEL_ARTIFACT_PATH
FIXTURE_IDS = [
    "all_passed",
    "failed_tests",
    "incomplete_jobs",
...
REQUIRED_FILES = [
    ...
    "artifacts/state/post_mvp_run_event_v1_catalog.json",
    ...
    "artifacts/runs/post_mvp_test_execution_run.json",
    "artifacts/steps/post_mvp_test_execution_step_list.json",
    "artifacts/tool_calls/post_mvp_test_execution_tool_call_list.json",
    "artifacts/intents/post_mvp_test_execution_intent.json",
...
PLAN_REQUIRED_MARKERS = [
    ...
    "RunV1",
    "run-artifact-v1",
    "post_mvp_test_execution_run.json",
    "StepListV1",
    "step-list-artifact-v1",
    "step-v1",
    "post_mvp_test_execution_step_list.json",
    "ToolCallListV1",
    "tool-call-list-artifact-v1",
    "tool-call-v1",
    "post_mvp_test_execution_tool_call_list.json",
    "IntentV1",
    "intent-v1",
    "post_mvp_test_execution_intent.json",
    "CapabilityV1",
    "capability-v1",
    "capability-item-v1",
    ...
    "run-event-v1",
    "run-event-item-v1",
    "post_mvp_run_event_v1_catalog.json",
...
def expect_evidence_list_artifact_validation_failure(
    label: str,
    manifest: dict,
    expected_message_fragment: str,
) -> None:
    try:
        ...
        if expected_message_fragment not in message:
            raise AssertionError(
                f"EvidenceListV1 forced-failure case {label} failed for the wrong reason.\n"
                f"Expected message fragment: {expected_message_fragment}\n"
                f"Actual message: {message}"
            ) from exc
        return
    raise AssertionError(
        f"EvidenceListV1 forced-failure case {label} unexpectedly passed validation"
    )


def validate_committed_evidence_list_artifact() -> None:
    artifact = load_json(EVIDENCE_LIST_ARTIFACT_PATH)
    try:
        validate_evidence_list_artifact(artifact, ROOT)
    except ValueError as exc:
        raise AssertionError(
            f"Committed EvidenceListV1 artifact failed validation: {exc}"
        ) from exc

    expected_artifact = build_evidence_list_artifact(ROOT)
    if artifact != expected_artifact:
        raise AssertionError(
            "Committed EvidenceListV1 artifact differs from deterministic builder output"
        )

    tampered = json.loads(json.dumps(artifact))
    tampered["workloadType"] = "read_only_regression_evidence_demo"
    expect_evidence_list_artifact_validation_failure(
        "workload_type_regression_reuse",
        tampered,
        "workloadType must be test_execution_failure_triage",
    )
    ...
    expect_evidence_list_artifact_validation_failure(
        "schema_regression_reuse",
        tampered,
        "schemaVersion must be evidence-list-artifact-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceKindDomain"] = ["test_case_outcome_record"]
    expect_evidence_list_artifact_validation_failure(
        "evidence_kind_domain_drift",
        tampered,
        "evidenceKindDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceSourceKindDomain"] = ["test_execution_result_v1"]
    expect_evidence_list_artifact_validation_failure(
        "evidence_source_kind_domain_drift",
        tampered,
        "evidenceSourceKindDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"] = []
    expect_evidence_list_artifact_validation_failure(
        "evidence_items_empty",
        tampered,
        "evidenceItems must be a non-empty list",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["schemaVersion"] = "regression-evidence-item-v1"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_schema_regression_reuse",
        tampered,
        "schemaVersion must be evidence-item-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["evidenceKind"] = "regression_evidence_marker"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_kind_not_in_domain",
        tampered,
        "evidenceKind must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][1] = json.loads(json.dumps(tampered["evidenceItems"][0]))
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_duplicate_id",
        tampered,
        "duplicate evidenceItems.evidenceId",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["contentHash"] = "0" * 64
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_content_hash_drift",
        tampered,
        "contentHash must equal TestExecutionResultV1.evidenceList contentHash",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["evidenceId"] = "ev-unknown-id"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_id_missing_from_upstream",
        tampered,
        "is not present in TestExecutionResultV1.evidenceList",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["capabilityRef"] = "write_artifact"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_capability_kind_mismatch",
        tampered,
        "capabilityRef for kind",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["capabilityRef"] = "send_email_capability"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_capability_unknown",
        tampered,
        "capabilityRef must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["redactionApplied"] = True
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_redaction_applied",
        tampered,
        "redactionApplied must be false",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["redactionPolicyRef"] = "artifacts/runs/all_passed/regression_result.json"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_redaction_policy_regression",
        tampered,
        "redactionPolicyRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["source"]["sourceKind"] = "run_event_log_v1"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_source_kind_wrong",
        tampered,
        "source.sourceKind must equal test_execution_result_v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["source"]["sourceArtifactSchemaVersion"] = "regression-result-artifact-v1"
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_source_schema_regression_reuse",
        tampered,
        "source.sourceArtifactSchemaVersion must equal test-execution-result-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"][0]["source"]["sourceArtifactContentHash"] = "0" * 64
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_source_hash_drift",
        tampered,
        "sourceArtifactContentHash must match",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItems"] = tampered["evidenceItems"][:-1]
    expect_evidence_list_artifact_validation_failure(
        "evidence_items_missing_upstream_item",
        tampered,
        "must mirror TestExecutionResultV1.evidenceList",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceItemCount"] = 0
    expect_evidence_list_artifact_validation_failure(
        "evidence_item_count_drift",
        tampered,
        "evidenceItemCount must equal len(evidenceItems)",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["testExecutionResultBinding"]["testExecutionResultSchemaVersion"] = "regression-result-artifact-v1"
    expect_evidence_list_artifact_validation_failure(
        "test_execution_result_binding_regression_schema_reuse",
        tampered,
        "testExecutionResultSchemaVersion must equal test-execution-result-v1",
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["failureTriageReportBinding"]["failureTriageReportSchemaVersion"] = "regression-result-artifact-v1"
    expect_evidence_list_artifact_validation_failure(
        "failure_triage_report_binding_regression_schema_reuse",
        tampered,
        "failureTriageReportSchemaVersion must equal failure-triage-report-v1",
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["failureTriageEvidenceCoverage"]["evidenceListCoversTriage"] = False
    expect_evidence_list_artifact_validation_failure(
        "failure_triage_coverage_false_flag",
        tampered,
        "evidenceListCoversTriage must equal triageEvidenceIds",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["failureTriageEvidenceCoverage"]["triageEvidenceIds"] = []
    expect_evidence_list_artifact_validation_failure(
        "failure_triage_coverage_empty_ids",
        tampered,
        "triageEvidenceIds must equal the sorted set",
    )
    ...
    for hash_role, label in [
        ...
        ("test_execution_capability_manifest", "source_hash_mismatch_capability_manifest"),
        ("test_execution_run_event_log", "source_hash_mismatch_run_event_log"),
        ("test_execution_result", "source_hash_mismatch_test_execution_result"),
        ("test_execution_verifier_result", "source_hash_mismatch_verifier_result"),
        ("test_execution_failure_triage_report", "source_hash_mismatch_failure_triage"),
        ("test_execution_delivery_manifest", "source_hash_mismatch_delivery_manifest"),
...
def run_evidence_list_artifact_builder(artifact_out: Path) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts/evidence_artifact.py"),
        "--out",
        str(artifact_out),
    ]
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=False)
...
def expect_run_artifact_validation_failure(
    label: str,
    manifest: dict,
    expected_message_fragment: str,
) -> None:
    try:
        ...
        if expected_message_fragment not in message:
            raise AssertionError(
                f"RunV1 forced-failure case {label} failed for the wrong reason.\n"
                f"Expected message fragment: {expected_message_fragment}\n"
                f"Actual message: {message}"
            ) from exc
        return
    raise AssertionError(
        f"RunV1 forced-failure case {label} unexpectedly passed validation"
    )


def validate_committed_run_artifact() -> None:
    artifact = load_json(RUN_ARTIFACT_PATH)
    try:
        validate_run_artifact(artifact, ROOT)
    except ValueError as exc:
        raise AssertionError(
            f"Committed RunV1 artifact failed validation: {exc}"
        ) from exc

    expected_artifact = build_run_artifact(ROOT)
    if artifact != expected_artifact:
        raise AssertionError(
            "Committed RunV1 artifact differs from deterministic builder output"
        )

    tampered = json.loads(json.dumps(artifact))
    tampered["workloadType"] = "read_only_regression_evidence_demo"
    expect_run_artifact_validation_failure(
        "workload_type_regression_reuse",
        tampered,
        "workloadType must be test_execution_failure_triage",
    )
    ...
    expect_run_artifact_validation_failure(
        "schema_regression_reuse",
        tampered,
        "schemaVersion must be run-artifact-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["runId"] = "run-regression-all-passed"
    expect_run_artifact_validation_failure(
        "run_id_regression_reuse",
        tampered,
        "runId must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runState"] = "failed"
    expect_run_artifact_validation_failure(
        "run_state_terminal_drift",
        tampered,
        "runState must equal RunEventLogV1.terminalState",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["startedAt"] = 99999
    expect_run_artifact_validation_failure(
        "started_at_drift",
        tampered,
        "startedAt must equal RunEventLogV1.events[0]",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["terminalAt"] = 0
    expect_run_artifact_validation_failure(
        "terminal_at_drift",
        tampered,
        "terminalAt must equal RunEventLogV1.events[-1]",
    )
    ...
    expect_run_artifact_validation_failure(
        "event_count_drift",
        tampered,
        "eventCount must equal RunEventLogV1.eventCount",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["observationCount"] = 1
    expect_run_artifact_validation_failure(
        "observation_count_drift",
        tampered,
        "observationCount must equal ObservationListV1.observationItemCount",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceCount"] = 1
    expect_run_artifact_validation_failure(
        "evidence_count_drift",
        tampered,
        "evidenceCount must equal EvidenceListV1.evidenceItemCount",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runControlStateMachine"]["runControlStateMachineSchemaVersion"] = "regression-state-machine-v1"
    expect_run_artifact_validation_failure(
        "run_state_machine_schema_regression",
        tampered,
        "runControlStateMachineSchemaVersion must equal run-state-machine-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runControlStateMachine"]["allowedRunControlTransitions"] = [["created", "completed"]]
    expect_run_artifact_validation_failure(
        "run_state_machine_allowed_drift",
        tampered,
        "runControlStateMachine.allowedRunControlTransitions must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runControlStateMachine"]["observedRunControlTransitions"] = []
    expect_run_artifact_validation_failure(
        "run_state_machine_observed_drift",
        tampered,
        "runControlStateMachine.observedRunControlTransitions",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runControlStateMachine"]["initialRunControlState"] = "running"
    expect_run_artifact_validation_failure(
        "run_state_machine_initial_drift",
        tampered,
        "runControlStateMachine.initialRunControlState must equal",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["observationListBinding"]["observationListSchemaVersion"] = "regression-observation-list-v1"
    expect_run_artifact_validation_failure(
        "observation_list_binding_regression_schema_reuse",
        tampered,
        "observationListSchemaVersion must equal observation-list-artifact-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["observationListBinding"]["observationListContentHash"] = "0" * 64
    expect_run_artifact_validation_failure(
        "observation_list_binding_hash_drift",
        tampered,
        "observationListContentHash must match",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["observationListBinding"]["observationItemCount"] = 1
    expect_run_artifact_validation_failure(
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["approvalDecisionBinding"]["approvalDecisionSchemaVersion"] = "regression-approval-v1"
    expect_run_artifact_validation_failure(
        "approval_decision_binding_schema_drift",
        tampered,
        "approvalDecisionSchemaVersion must equal human-approval-decision-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["approvalDecisionBinding"]["approvalDecisionContentHash"] = "0" * 64
    expect_run_artifact_validation_failure(
        "approval_decision_binding_hash_drift",
        tampered,
        "approvalDecisionContentHash must match",
    )
    ...
    expect_run_artifact_validation_failure(
        "reuses_regression_run_artifact",
        tampered,
        "must not reuse the first-workload regression run.json",
    )
...
def run_run_artifact_builder(artifact_out: Path) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts/run_artifact.py"),
        "--out",
        str(artifact_out),
    ]
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=False)
...
def expect_step_list_artifact_validation_failure(
    label: str,
    manifest: dict,
    expected_message_fragment: str,
) -> None:
    try:
        ...
        if expected_message_fragment not in message:
            raise AssertionError(
                f"StepListV1 forced-failure case {label} failed for the wrong reason.\n"
                f"Expected message fragment: {expected_message_fragment}\n"
                f"Actual message: {message}"
            ) from exc
        return
    raise AssertionError(
        f"StepListV1 forced-failure case {label} unexpectedly passed validation"
    )


def validate_committed_step_list_artifact() -> None:
    artifact = load_json(STEP_LIST_ARTIFACT_PATH)
    try:
        validate_step_list_artifact(artifact, ROOT)
    except ValueError as exc:
        raise AssertionError(
            f"Committed StepListV1 artifact failed validation: {exc}"
        ) from exc

    expected_artifact = build_step_list_artifact(ROOT)
    if artifact != expected_artifact:
        raise AssertionError(
            "Committed StepListV1 artifact differs from deterministic builder output"
        )

    tampered = json.loads(json.dumps(artifact))
    tampered["workloadType"] = "read_only_regression_evidence_demo"
    expect_step_list_artifact_validation_failure(
        "workload_type_regression_reuse",
        tampered,
        "workloadType must be test_execution_failure_triage",
    )
    ...
    expect_step_list_artifact_validation_failure(
        "schema_regression_reuse",
        tampered,
        "schemaVersion must be step-list-artifact-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["stepItemSchemaVersion"] = "step-v0"
    expect_step_list_artifact_validation_failure(
        "step_item_schema_version_drift",
        tampered,
        "stepItemSchemaVersion must be step-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["stepKindDomain"] = ["capability_invocation_step"]
    expect_step_list_artifact_validation_failure(
        "step_kind_domain_drift",
        tampered,
        "stepKindDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepStatusDomain"] = ["passed", "failed"]
    expect_step_list_artifact_validation_failure(
        "step_status_domain_drift",
        tampered,
        "stepStatusDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["sourceAgentDomain"] = ["creator_agent"]
    expect_step_list_artifact_validation_failure(
        "source_agent_domain_drift",
        tampered,
        "sourceAgentDomain must equal",
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["runKernelCapabilitySentinel"] = "ide_terminal"
    expect_step_list_artifact_validation_failure(
        "run_kernel_capability_sentinel_drift",
        tampered,
        "runKernelCapabilitySentinel must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runIdentifier"] = "run-regression-all-passed"
    expect_step_list_artifact_validation_failure(
        "run_identifier_regression_reuse",
        tampered,
        "runIdentifier must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepCount"] = 7
    expect_step_list_artifact_validation_failure(
        "step_count_drift",
        tampered,
        "stepCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"] = tampered["stepItems"][:-1]
    expect_step_list_artifact_validation_failure(
        "step_items_length_drift",
        tampered,
        "stepItems must be a list of length",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["stepKind"] = "verifier_step"
    expect_step_list_artifact_validation_failure(
        "step_item_kind_blueprint_drift",
        tampered,
        "stepKind must equal blueprint stepKind",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["stepKind"] = "totally_invalid_kind"
    expect_step_list_artifact_validation_failure(
        "step_item_kind_domain_drift",
        tampered,
        "stepKind must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["status"] = "unknown_status"
    expect_step_list_artifact_validation_failure(
        "step_item_status_domain_drift",
        tampered,
        "status must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["sourceAgent"] = "unknown_agent"
    expect_step_list_artifact_validation_failure(
        "step_item_source_agent_domain_drift",
        tampered,
        "sourceAgent must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["capabilityRef"] = "send_email"
    expect_step_list_artifact_validation_failure(
        "step_item_capability_ref_drift",
        tampered,
        "capabilityRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["schemaVersion"] = "step-v2"
    expect_step_list_artifact_validation_failure(
        "step_item_schema_version_drift",
        tampered,
        "schemaVersion must be step-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["sequence"] = 99
    expect_step_list_artifact_validation_failure(
        "step_item_sequence_drift",
        tampered,
        "sequence must equal its position",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["stepId"] = "step-regression-all-passed-0002"
    expect_step_list_artifact_validation_failure(
        "step_item_step_id_regression_reuse",
        tampered,
        "stepId must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["runEventLogRefs"] = []
    expect_step_list_artifact_validation_failure(
        "step_item_run_event_log_refs_empty",
        tampered,
        "runEventLogRefs must be a non-empty list",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][3]["runEventLogRefs"] = [
        "evt-run-post-mvp-test-execution-failure-triage-9999",
        "evt-run-post-mvp-test-execution-failure-triage-9998",
    ]
    expect_step_list_artifact_validation_failure(
        "step_item_run_event_log_ref_unknown",
        tampered,
        "runEventLogRefs must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][3]["observationRefs"] = [
        "obs-run-post-mvp-test-execution-failure-triage-9999",
        "obs-run-post-mvp-test-execution-failure-triage-9998",
    ]
    expect_step_list_artifact_validation_failure(
        "step_item_observation_ref_unknown",
        tampered,
        "references unknown ObservationListV1.observationId",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][3]["observationRefs"] = []
    expect_step_list_artifact_validation_failure(
        "step_item_observation_refs_length_drift",
        tampered,
        "observationRefs must mirror runEventLogRefs length",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][3]["evidenceRefs"] = ["ev-case-001-outcome"]
    expect_step_list_artifact_validation_failure(
        "step_item_evidence_refs_blueprint_drift",
        tampered,
        "evidenceRefs must equal blueprint evidenceIds",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][3]["evidenceRefs"] = [
        "ev-case-001-outcome",
        "ev-case-002-outcome",
        "ev-case-002-trace",
        "ev-case-999-missing",
    ...
    expect_step_list_artifact_validation_failure(
        "step_item_evidence_ref_unknown",
        tampered,
        "evidenceRefs must equal blueprint evidenceIds",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["startedAt"] = 99999
    expect_step_list_artifact_validation_failure(
        "step_item_started_at_drift",
        tampered,
        "startedAt must equal first event timestampMs",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["completedAt"] = 0
    expect_step_list_artifact_validation_failure(
        "step_item_completed_at_drift",
        tampered,
        "completedAt must equal last event timestampMs",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["inputContract"] = "ReadLogInputV1"
    expect_step_list_artifact_validation_failure(
        "step_item_input_contract_drift",
        tampered,
        "inputContract must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepItems"][2]["outputContract"] = "ReadLogOutputV1"
    expect_step_list_artifact_validation_failure(
        "step_item_output_contract_drift",
        tampered,
        "outputContract must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventCoverage"]["everyEventCovered"] = False
    expect_step_list_artifact_validation_failure(
        "run_event_coverage_every_event_false",
        tampered,
        "runEventCoverage.everyEventCovered must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventCoverage"]["eventCount"] = 1
    expect_step_list_artifact_validation_failure(
        "run_event_coverage_count_drift",
        tampered,
        "runEventCoverage.eventCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventCoverage"]["missingEvents"] = ["evt-foo"]
    expect_step_list_artifact_validation_failure(
        "run_event_coverage_missing_events_not_empty",
        tampered,
        "runEventCoverage.missingEvents must be empty",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventCoverage"]["duplicateEvents"] = ["evt-foo"]
    expect_step_list_artifact_validation_failure(
        "run_event_coverage_duplicate_events_not_empty",
        tampered,
        "runEventCoverage.duplicateEvents must be empty",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["observationCoverage"]["everyObservationCovered"] = False
    expect_step_list_artifact_validation_failure(
        "observation_coverage_every_observation_false",
        tampered,
        "observationCoverage.everyObservationCovered must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["observationCoverage"]["observationItemCount"] = 1
    expect_step_list_artifact_validation_failure(
        "observation_coverage_count_drift",
        tampered,
        "observationCoverage.observationItemCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceCoverage"]["everyEvidenceItemCovered"] = False
    expect_step_list_artifact_validation_failure(
        "evidence_coverage_every_evidence_false",
        tampered,
        "evidenceCoverage.everyEvidenceItemCovered must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["evidenceCoverage"]["evidenceItemCount"] = 99
    expect_step_list_artifact_validation_failure(
        "evidence_coverage_count_drift",
        tampered,
        "evidenceCoverage.evidenceItemCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runBinding"]["runSchemaVersion"] = "regression-run-v1"
    expect_step_list_artifact_validation_failure(
        "run_binding_regression_schema_reuse",
        tampered,
        "runBinding.runSchemaVersion must equal run-artifact-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runBinding"]["runContentHash"] = "0" * 64
    expect_step_list_artifact_validation_failure(
        "run_binding_hash_drift",
        tampered,
        "runBinding.runContentHash must match",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runBinding"]["runIdentifier"] = "run-regression-all-passed"
    expect_step_list_artifact_validation_failure(
        "run_binding_identifier_drift",
        tampered,
        "runBinding.runIdentifier must equal RunV1.runId",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["runBinding"]["eventCount"] = 99
    expect_step_list_artifact_validation_failure(
        "run_binding_event_count_drift",
        tampered,
        "runBinding.eventCount must equal RunV1.eventCount",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["regressionWorkloadIsolation"]["reusesRegressionRunStepArray"] = True
    expect_step_list_artifact_validation_failure(
        "reuses_regression_run_step_array",
        tampered,
        "must not reuse the first-workload regression run.json steps array",
    )
...
def run_step_list_artifact_builder(artifact_out: Path) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts/step_list_artifact.py"),
        "--out",
        str(artifact_out),
    ]
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=False)
...
def expect_tool_call_list_artifact_validation_failure(
    label: str,
    manifest: dict,
    expected_message_fragment: str,
) -> None:
    try:
        ...
        if expected_message_fragment not in message:
            raise AssertionError(
                f"ToolCallListV1 forced-failure case {label} failed for the wrong reason.\n"
                f"Expected message fragment: {expected_message_fragment}\n"
                f"Actual message: {message}"
            ) from exc
        return
    raise AssertionError(
        f"ToolCallListV1 forced-failure case {label} unexpectedly passed validation"
    )


def validate_committed_tool_call_list_artifact() -> None:
    artifact = load_json(TOOL_CALL_LIST_ARTIFACT_PATH)
    try:
        validate_tool_call_list_artifact(artifact, ROOT)
    except ValueError as exc:
        raise AssertionError(
            f"Committed ToolCallListV1 artifact failed validation: {exc}"
        ) from exc

    expected_artifact = build_tool_call_list_artifact(ROOT)
    if artifact != expected_artifact:
        raise AssertionError(
            "Committed ToolCallListV1 artifact differs from deterministic builder output"
        )

    tampered = json.loads(json.dumps(artifact))
    tampered["workloadType"] = "read_only_regression_evidence_demo"
    expect_tool_call_list_artifact_validation_failure(
        "workload_type_regression_reuse",
        tampered,
        "workloadType must be test_execution_failure_triage",
    )
    ...
    expect_tool_call_list_artifact_validation_failure(
        "schema_regression_reuse",
        tampered,
        "schemaVersion must be tool-call-list-artifact-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItemSchemaVersion"] = "tool-call-v0"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_schema_version_drift",
        tampered,
        "toolCallItemSchemaVersion must be tool-call-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["sideEffectClassDomain"] = ["read_only"]
    expect_tool_call_list_artifact_validation_failure(
        "side_effect_class_domain_drift",
        tampered,
        "sideEffectClassDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["forbiddenSideEffectClasses"] = ["external"]
    expect_tool_call_list_artifact_validation_failure(
        "forbidden_side_effect_classes_drift",
        tampered,
        "forbiddenSideEffectClasses must equal",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["toolBearingStepKinds"] = ["capability_invocation_step"]
    expect_tool_call_list_artifact_validation_failure(
        "tool_bearing_step_kinds_drift",
        tampered,
        "toolBearingStepKinds must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallStatusDomain"] = ["passed"]
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_status_domain_drift",
        tampered,
        "toolCallStatusDomain must equal",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallCount"] = 4
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_count_drift",
        tampered,
        "toolCallCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"] = tampered["toolCallItems"][:-1]
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_items_length_drift",
        tampered,
        "toolCallItems must be a list of length",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][0]["schemaVersion"] = "tool-call-v2"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_schema_drift",
        tampered,
        "schemaVersion must be tool-call-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["sequence"] = 99
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_sequence_drift",
        tampered,
        "sequence must equal its position",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["toolCallId"] = "toolcall-regression-all-passed-0001"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_id_regression_reuse",
        tampered,
        "toolCallId must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["stepRef"] = "step-run-post-mvp-test-execution-failure-triage-9999"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_step_ref_drift",
        tampered,
        "stepRef must equal StepListV1 tool-bearing step",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["stepKind"] = "run_state_transition_step"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_step_kind_non_tool_bearing",
        tampered,
        "stepKind must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["stepKind"] = "verifier_step"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_step_kind_mismatch",
        tampered,
        "stepKind must equal StepListV1 step.stepKind",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["capabilityRef"] = "totally_invalid_capability"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_capability_ref_drift",
        tampered,
        "capabilityRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["sourceAgent"] = "unknown_agent"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_source_agent_domain_drift",
        tampered,
        "sourceAgent must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["sourceAgent"] = "verifier-runtime-v1"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_source_agent_blueprint_drift",
        tampered,
        "sourceAgent must equal creator_agent",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["status"] = "passed"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_status_domain_drift",
        tampered,
        "status must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["startedAt"] = 99999
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_started_at_drift",
        tampered,
        "startedAt must equal StepListV1 step.startedAt",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["completedAt"] = 0
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_completed_at_drift",
        tampered,
        "completedAt must equal StepListV1 step.completedAt",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["sideEffectClass"] = "external"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_side_effect_class_forbidden",
        tampered,
        "sideEffectClass must not be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["sideEffectClass"] = "unknown_class"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_side_effect_class_domain_drift",
        tampered,
        "sideEffectClass must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["sideEffectClass"] = "read_only"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_side_effect_class_blueprint_drift",
        tampered,
        "sideEffectClass must equal evidence_writer",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["inputSchemaRef"] = "ReadLogInputV1"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_input_schema_ref_drift",
        tampered,
        "inputSchemaRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["outputSchemaRef"] = "ReadLogOutputV1"
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_output_schema_ref_drift",
        tampered,
        "outputSchemaRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["inputPayloadHash"] = "0" * 64
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_input_payload_hash_drift",
        tampered,
        "inputPayloadHash must equal sha256",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["outputPayloadHash"] = "0" * 64
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_output_payload_hash_drift",
        tampered,
        "outputPayloadHash must equal sha256",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["inputPayloadShape"] = {"type": "Tampered"}
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_input_payload_shape_drift",
        tampered,
        "inputPayloadShape must equal blueprint inputPayloadShape",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["outputPayloadShape"] = {"type": "Tampered"}
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_output_payload_shape_drift",
        tampered,
        "outputPayloadShape must equal blueprint outputPayloadShape",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["runEventLogRefs"] = ["evt-foo"]
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_run_event_log_refs_drift",
        tampered,
        "runEventLogRefs must equal StepListV1 step.runEventLogRefs",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallItems"][1]["observationRefs"] = ["obs-foo"]
    expect_tool_call_list_artifact_validation_failure(
        "tool_call_item_observation_refs_drift",
        tampered,
        "observationRefs must equal StepListV1 step.observationRefs",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepCoverage"]["everyToolBearingStepCovered"] = False
    expect_tool_call_list_artifact_validation_failure(
        "step_coverage_every_step_false",
        tampered,
        "stepCoverage.everyToolBearingStepCovered must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepCoverage"]["toolBearingStepCount"] = 1
    expect_tool_call_list_artifact_validation_failure(
        "step_coverage_tool_bearing_count_drift",
        tampered,
        "stepCoverage.toolBearingStepCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepCoverage"]["missingSteps"] = ["step-foo"]
    expect_tool_call_list_artifact_validation_failure(
        "step_coverage_missing_steps_not_empty",
        tampered,
        "stepCoverage.missingSteps must be empty",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepCoverage"]["duplicateSteps"] = ["step-foo"]
    expect_tool_call_list_artifact_validation_failure(
        "step_coverage_duplicate_steps_not_empty",
        tampered,
        "stepCoverage.duplicateSteps must be empty",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepListBinding"]["stepListSchemaVersion"] = "regression-run-step-array-v1"
    expect_tool_call_list_artifact_validation_failure(
        "step_list_binding_regression_schema_reuse",
        tampered,
        "stepListBinding.stepListSchemaVersion must equal step-list-artifact-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepListBinding"]["stepListContentHash"] = "0" * 64
    expect_tool_call_list_artifact_validation_failure(
        "step_list_binding_hash_drift",
        tampered,
        "stepListBinding.stepListContentHash must match",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["stepListBinding"]["toolBearingStepCount"] = 1
    expect_tool_call_list_artifact_validation_failure(
        "step_list_binding_tool_bearing_count_drift",
        tampered,
        "stepListBinding.toolBearingStepCount must equal",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["regressionWorkloadIsolation"]["reusesRegressionToolCallArtifact"] = True
    expect_tool_call_list_artifact_validation_failure(
        "reuses_regression_tool_call_artifact",
        tampered,
        "must not reuse any first-workload regression tool-call artifact",
    )
...
def run_tool_call_list_artifact_builder(artifact_out: Path) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts/tool_call_list_artifact.py"),
        "--out",
        str(artifact_out),
    ]
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=False)
...
def expect_intent_artifact_validation_failure(
    label: str,
    manifest: dict,
    expected_message_fragment: str,
) -> None:
    try:
        ...
        if expected_message_fragment not in message:
            raise AssertionError(
                f"IntentV1 forced-failure case {label} failed for the wrong reason.\n"
                f"Expected message fragment: {expected_message_fragment}\n"
                f"Actual message: {message}"
            ) from exc
        return
    raise AssertionError(
        f"IntentV1 forced-failure case {label} unexpectedly passed validation"
    )


def validate_committed_intent_artifact() -> None:
    artifact = load_json(INTENT_ARTIFACT_PATH)
    try:
        validate_intent_artifact(artifact, ROOT)
    except ValueError as exc:
        raise AssertionError(
            f"Committed IntentV1 artifact failed validation: {exc}"
        ) from exc

    expected_artifact = build_intent_artifact(ROOT)
    if artifact != expected_artifact:
        raise AssertionError(
            "Committed IntentV1 artifact differs from deterministic builder output"
        )

    tampered = json.loads(json.dumps(artifact))
    tampered["workloadType"] = "read_only_regression_evidence_demo"
    expect_intent_artifact_validation_failure(
        "workload_type_regression_reuse",
        tampered,
        "workloadType must be test_execution_failure_triage",
    )
    ...
    expect_intent_artifact_validation_failure(
        "schema_regression_reuse",
        tampered,
        "schemaVersion must be intent-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["intentLifecycleState"] = "drafted"
    expect_intent_artifact_validation_failure(
        "lifecycle_state_drafted_drift",
        tampered,
        "intentLifecycleState must equal accepted_into_task_spec",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["intentLifecycleState"] = "unknown_state"
    expect_intent_artifact_validation_failure(
        "lifecycle_state_outside_domain",
        tampered,
        "intentLifecycleState must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["intentLifecycleStateDomain"] = ["drafted"]
    expect_intent_artifact_validation_failure(
        "lifecycle_state_domain_drift",
        tampered,
        "intentLifecycleStateDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["acceptedTaskSpecRef"] = "regression-task-spec-all-passed"
    expect_intent_artifact_validation_failure(
        "accepted_task_spec_ref_regression_reuse",
        tampered,
        "acceptedTaskSpecRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedWorkloadType"] = "read_only_regression_evidence_demo"
    expect_intent_artifact_validation_failure(
        "requested_workload_type_regression_reuse",
        tampered,
        "requestedWorkloadType must equal test_execution_failure_triage",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedCapabilityClasses"] = [
        "read_only_inspection",
        "external_communication",
    ]
    expect_intent_artifact_validation_failure(
        "requested_capabilities_include_forbidden",
        tampered,
        "requestedCapabilityClasses contains forbidden class",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedCapabilityClasses"] = ["read_only_inspection", "unknown_class"]
    expect_intent_artifact_validation_failure(
        "requested_capabilities_outside_domain",
        tampered,
        "requestedCapabilityClasses contains class outside",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedCapabilityClasses"] = []
    expect_intent_artifact_validation_failure(
        "requested_capabilities_empty",
        tampered,
        "requestedCapabilityClasses must be a non-empty list",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["forbiddenRequestedCapabilityClasses"] = ["external_communication"]
    expect_intent_artifact_validation_failure(
        "forbidden_requested_capabilities_drift",
        tampered,
        "forbiddenRequestedCapabilityClasses must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedCapabilityClassDomain"] = ["read_only_inspection"]
    expect_intent_artifact_validation_failure(
        "requested_capability_class_domain_drift",
        tampered,
        "requestedCapabilityClassDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedArtifactKinds"] = ["UnknownKindV1"]
    expect_intent_artifact_validation_failure(
        "requested_artifact_kinds_outside_domain",
        tampered,
        "requestedArtifactKinds contains kind outside",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedVerifierClass"] = "creator_agent"
    expect_intent_artifact_validation_failure(
        "requested_verifier_class_drift",
        tampered,
        "requestedVerifierClass must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedDeliveryClass"] = "external_email_delivery"
    expect_intent_artifact_validation_failure(
        "requested_delivery_class_drift",
        tampered,
        "requestedDeliveryClass must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requestedPolicyClass"] = "regression_policy"
    expect_intent_artifact_validation_failure(
        "requested_policy_class_drift",
        tampered,
        "requestedPolicyClass must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["intentOriginAgent"] = "unknown_agent"
    expect_intent_artifact_validation_failure(
        "intent_origin_agent_outside_domain",
        tampered,
        "intentOriginAgent must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["intentOriginChannel"] = "unknown_channel"
    expect_intent_artifact_validation_failure(
        "intent_origin_channel_outside_domain",
        tampered,
        "intentOriginChannel must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["intentOriginAgentDomain"] = ["creator_agent"]
    expect_intent_artifact_validation_failure(
        "intent_origin_agent_domain_drift",
        tampered,
        "intentOriginAgentDomain must equal",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["stepListBinding"]["stepCount"] = 1
    expect_intent_artifact_validation_failure(
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallListBinding"]["toolCallListSchemaVersion"] = "regression-tool-call-v1"
    expect_intent_artifact_validation_failure(
        "tool_call_list_binding_regression_schema_reuse",
        tampered,
        "toolCallListSchemaVersion must equal tool-call-list-artifact-v1",
    )
    ...
    tampered["capabilityManifestBinding"]["capabilityManifestSchemaVersion"] = "capability-envelope-v1"
    expect_intent_artifact_validation_failure(
        "capability_manifest_binding_schema_reuse",
        tampered,
        "capabilityManifestSchemaVersion must equal capability-manifest-v1",
    )
    ...
    tampered["runEventLogBinding"]["runEventLogSchemaVersion"] = "regression-events-v1"
    expect_intent_artifact_validation_failure(
        "run_event_log_binding_schema_reuse",
        tampered,
        "runEventLogSchemaVersion must equal run-event-log-v1",
    )
    ...
    tampered["observationListBinding"]["observationListSchemaVersion"] = "regression-observation-list-v1"
    expect_intent_artifact_validation_failure(
        "observation_list_binding_schema_reuse",
        tampered,
        "observationListSchemaVersion must equal observation-list-artifact-v1",
    )
    ...
    tampered["evidenceListBinding"]["evidenceListSchemaVersion"] = "evidence-list-v1"
    expect_intent_artifact_validation_failure(
        "evidence_list_binding_schema_reuse",
        tampered,
        "evidenceListSchemaVersion must equal evidence-list-artifact-v1",
    )
    ...
    tampered["verifierResultBinding"]["verifierResultSchemaVersion"] = "regression-result-artifact-v1"
    expect_intent_artifact_validation_failure(
        "verifier_result_binding_schema_reuse",
        tampered,
        "verifierResultSchemaVersion must equal verifier-result-v1",
    )
    ...
    tampered["deliveryManifestBinding"]["deliveryManifestSchemaVersion"] = "multi-agent-delivery-manifest-v1"
    expect_intent_artifact_validation_failure(
        "delivery_manifest_binding_schema_reuse",
        tampered,
        "deliveryManifestSchemaVersion must equal delivery-manifest-v1",
    )
    ...
    tampered["policyManifestBinding"]["policyManifestSchemaVersion"] = "adapter-policy-manifest-v1"
    expect_intent_artifact_validation_failure(
        "policy_manifest_binding_schema_reuse",
        tampered,
        "policyManifestSchemaVersion must equal policy-manifest-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["promotionConditions"]["requiredLifecycleState"] = "drafted"
    expect_intent_artifact_validation_failure(
        "promotion_required_lifecycle_state_drift",
        tampered,
        "promotionConditions.requiredLifecycleState must be accepted_into_task_spec",
    )
    ...
    expect_intent_artifact_validation_failure(
        "promotion_unlocked_when_blocked",
        tampered,
        "promotionConditions.promoted must be true only when lifecycle",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["regressionWorkloadIsolation"]["reusesRegressionIntentArtifact"] = True
    expect_intent_artifact_validation_failure(
        "reuses_regression_intent_artifact",
        tampered,
        "must not reuse any first-workload regression intent artifact",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["regressionWorkloadIsolation"]["reusesRegressionTaskSpec"] = True
    expect_intent_artifact_validation_failure(
        "reuses_regression_task_spec",
        tampered,
        "must not reuse the regression-task-spec-v1 schema",
    )
...
def run_intent_artifact_builder(artifact_out: Path) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts/intent_artifact.py"),
        "--out",
        str(artifact_out),
    ]
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=False)
...
def expect_capability_kernel_validation_failure(
    label: str,
    manifest: dict,
    expected_message_fragment: str,
) -> None:
    try:
        ...
        if expected_message_fragment not in message:
            raise AssertionError(
                f"CapabilityV1 forced-failure case {label} failed for the wrong reason.\n"
                f"Expected message fragment: {expected_message_fragment}\n"
                f"Actual message: {message}"
            ) from exc
        return
    raise AssertionError(
        f"CapabilityV1 forced-failure case {label} unexpectedly passed validation"
    )


def validate_committed_capability_kernel_artifact() -> None:
    artifact = load_json(CAPABILITY_KERNEL_ARTIFACT_PATH)
    try:
        validate_capability_kernel_artifact(artifact, ROOT)
    except ValueError as exc:
        raise AssertionError(
            f"Committed CapabilityV1 artifact failed validation: {exc}"
        ) from exc

    expected_artifact = build_capability_kernel_artifact(ROOT)
    if artifact != expected_artifact:
        raise AssertionError(
            "Committed CapabilityV1 artifact differs from deterministic builder output"
        )

    tampered = json.loads(json.dumps(artifact))
    tampered["workloadType"] = "read_only_regression_evidence_demo"
    expect_capability_kernel_validation_failure(
        "workload_type_regression_reuse",
        tampered,
        "workloadType must be test_execution_failure_triage",
    )
    ...
    expect_capability_kernel_validation_failure(
        "kind_regression_reuse",
        tampered,
        "kind must be CapabilityV1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["schemaVersion"] = "capability-manifest-v1"
    expect_capability_kernel_validation_failure(
        "schema_capability_manifest_reuse",
        tampered,
        "schemaVersion must be capability-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["artifactEnvelopeSchemaVersion"] = "capability-envelope-v1"
    expect_capability_kernel_validation_failure(
        "envelope_schema_capability_envelope_reuse",
        tampered,
        "artifactEnvelopeSchemaVersion must be artifact-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItemSchemaVersion"] = "capability-definition-v1"
    expect_capability_kernel_validation_failure(
        "item_schema_capability_definition_reuse",
        tampered,
        "capabilityItemSchemaVersion must be capability-item-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityCount"] = 99
    expect_capability_kernel_validation_failure(
        "capability_count_drift",
        tampered,
        "capabilityCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["capabilityClass"] = "external_communication"
    expect_capability_kernel_validation_failure(
        "capability_item_class_forbidden",
        tampered,
        "must not intersect declared capabilityItems",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["capabilityClass"] = "unknown_class"
    expect_capability_kernel_validation_failure(
        "capability_item_class_outside_domain",
        tampered,
        "capabilityClass outside workload-independent domain",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityClassDomain"] = ["read_only_inspection"]
    expect_capability_kernel_validation_failure(
        "capability_class_domain_drift",
        tampered,
        "capabilityClassDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["forbiddenCapabilityClasses"] = ["external_communication"]
    expect_capability_kernel_validation_failure(
        "forbidden_capability_classes_drift",
        tampered,
        "forbiddenCapabilityClasses must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["sideEffectClass"] = "email_send_call"
    expect_capability_kernel_validation_failure(
        "side_effect_class_forbidden",
        tampered,
        "sideEffectClass is forbidden",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["sideEffectClass"] = "unknown_side_effect"
    expect_capability_kernel_validation_failure(
        "side_effect_class_outside_domain",
        tampered,
        "sideEffectClass outside workload-independent domain",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityLifecycleStateDomain"] = ["drafted"]
    expect_capability_kernel_validation_failure(
        "lifecycle_state_domain_drift",
        tampered,
        "capabilityLifecycleStateDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requiredCapabilityLifecycleState"] = "drafted"
    expect_capability_kernel_validation_failure(
        "required_lifecycle_state_drift",
        tampered,
        "requiredCapabilityLifecycleState must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["capabilityLifecycleState"] = "drafted"
    expect_capability_kernel_validation_failure(
        "capability_item_lifecycle_state_drift",
        tampered,
        "capabilityLifecycleState must equal registered",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["capabilityLifecycleState"] = "unknown_state"
    expect_capability_kernel_validation_failure(
        "capability_item_lifecycle_state_outside_domain",
        tampered,
        "capabilityLifecycleState must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["permissionFlagOverrides"]["externalSideEffectsAllowed"] = True
    expect_capability_kernel_validation_failure(
        "capability_item_permission_override_true",
        tampered,
        "permissionFlagOverrides.externalSideEffectsAllowed must be false",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["sourceAgentDomain"] = ["unknown_agent"]
    expect_capability_kernel_validation_failure(
        "capability_item_source_agent_outside_domain",
        tampered,
        "sourceAgentDomain contains agent outside",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["sourceAgentDomain"] = ["verifier-runtime-v1"]
    expect_capability_kernel_validation_failure(
        "capability_item_source_agent_drift",
        tampered,
        "sourceAgentDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["verifierVerdictOwner"] = "creator_agent"
    expect_capability_kernel_validation_failure(
        "capability_item_verifier_owner_drift",
        tampered,
        "verifierVerdictOwner must equal verifier-runtime-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["inputSchemaRef"] = "UnknownInputSchemaV1"
    expect_capability_kernel_validation_failure(
        "capability_item_input_schema_ref_drift",
        tampered,
        "inputSchemaRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["outputSchemaRef"] = "UnknownOutputSchemaV1"
    expect_capability_kernel_validation_failure(
        "capability_item_output_schema_ref_drift",
        tampered,
        "outputSchemaRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["capabilityId"] = "unexpected_capability"
    expect_capability_kernel_validation_failure(
        "capability_item_id_drift",
        tampered,
        "capabilityId must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"][0]["sequence"] = 99
    expect_capability_kernel_validation_failure(
        "capability_item_sequence_drift",
        tampered,
        "sequence must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["capabilityItems"] = tampered["capabilityItems"][:-1]
    expect_capability_kernel_validation_failure(
        "capability_items_count_drift",
        tampered,
        "capabilityItems must contain exactly",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallCoverage"]["everyToolCallRefResolved"] = False
    expect_capability_kernel_validation_failure(
        "tool_call_coverage_unresolved_ref",
        tampered,
        "toolCallCoverage.everyToolCallRefResolved must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallCoverage"]["everyCapabilityItemUsed"] = False
    expect_capability_kernel_validation_failure(
        "tool_call_coverage_unused_capability",
        tampered,
        "toolCallCoverage.everyCapabilityItemUsed must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["toolCallCoverage"]["capabilityIdsCovered"] = ["read_test_execution_result"]
    expect_capability_kernel_validation_failure(
        "tool_call_coverage_capability_ids_drift",
        tampered,
        "toolCallCoverage.capabilityIdsCovered must mirror capabilityItems order",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["regressionWorkloadIsolation"]["reusesRegressionCapabilityArtifact"] = True
    expect_capability_kernel_validation_failure(
        "reuses_regression_capability_artifact",
        tampered,
        "must not reuse any first-workload regression capability artifact",
    )
...
def run_capability_kernel_builder(artifact_out: Path) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts/capability_kernel.py"),
        "--out",
        str(artifact_out),
    ]
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=False)
...
def expect_run_event_kernel_validation_failure(
    label: str,
    manifest: dict,
    expected_message_fragment: str,
) -> None:
    try:
        ...
        if expected_message_fragment not in message:
            raise AssertionError(
                f"RunEventV1 forced-failure case {label} failed for the wrong reason.\n"
                f"Expected message fragment: {expected_message_fragment}\n"
                f"Actual message: {message}"
            ) from exc
        return
    raise AssertionError(
        f"RunEventV1 forced-failure case {label} unexpectedly passed validation"
    )


def validate_committed_run_event_kernel_artifact() -> None:
    artifact = load_json(RUN_EVENT_KERNEL_ARTIFACT_PATH)
    try:
        validate_run_event_kernel_artifact(artifact, ROOT)
    except ValueError as exc:
        raise AssertionError(
            f"Committed RunEventV1 artifact failed validation: {exc}"
        ) from exc

    expected_artifact = build_run_event_kernel_artifact(ROOT)
    if artifact != expected_artifact:
        raise AssertionError(
            "Committed RunEventV1 artifact differs from deterministic builder output"
        )

    tampered = json.loads(json.dumps(artifact))
    tampered["workloadType"] = "read_only_regression_evidence_demo"
    expect_run_event_kernel_validation_failure(
        "workload_type_regression_reuse",
        tampered,
        "workloadType must be test_execution_failure_triage",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["kind"] = "RunEventLogV1"
    expect_run_event_kernel_validation_failure(
        "kind_run_event_log_reuse",
        tampered,
        "kind must be RunEventV1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["schemaVersion"] = "run-event-log-v1"
    expect_run_event_kernel_validation_failure(
        "schema_run_event_log_reuse",
        tampered,
        "schemaVersion must be run-event-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["artifactEnvelopeSchemaVersion"] = "run-event-envelope-v1"
    expect_run_event_kernel_validation_failure(
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItemSchemaVersion"] = "run-event-rule-v1"
    expect_run_event_kernel_validation_failure(
        "item_schema_drift",
        tampered,
        "runEventItemSchemaVersion must be run-event-item-v1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["runEventCount"] = 99
    expect_run_event_kernel_validation_failure(
        "run_event_count_drift",
        tampered,
        "runEventCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventClassDomain"] = ["run_lifecycle"]
    expect_run_event_kernel_validation_failure(
        "run_event_class_domain_drift",
        tampered,
        "runEventClassDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["forbiddenRunEventClasses"] = []
    expect_run_event_kernel_validation_failure(
        "forbidden_run_event_classes_drift",
        tampered,
        "forbiddenRunEventClasses must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItems"][0]["runEventClass"] = "regression_run_event"
    expect_run_event_kernel_validation_failure(
        "run_event_item_forbidden_class",
        tampered,
        "must not intersect declared runEventItems",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventScopeDomain"] = ["workload_independent_kernel"]
    expect_run_event_kernel_validation_failure(
        "run_event_scope_domain_drift",
        tampered,
        "runEventScopeDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["allowedCommittedRunEventScopes"] = ["workload_specific"]
    expect_run_event_kernel_validation_failure(
        "allowed_committed_scopes_drift",
        tampered,
        "allowedCommittedRunEventScopes must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItems"][0]["runEventScope"] = "workload_specific"
    expect_run_event_kernel_validation_failure(
        "run_event_item_workload_specific_scope",
        tampered,
        "runEventScope must be one of",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventLifecycleStateDomain"] = ["emitted"]
    expect_run_event_kernel_validation_failure(
        "lifecycle_state_domain_drift",
        tampered,
        "runEventLifecycleStateDomain must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["requiredRunEventLifecycleState"] = "drafted"
    expect_run_event_kernel_validation_failure(
        "required_lifecycle_state_drift",
        tampered,
        "requiredRunEventLifecycleState must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItems"][0]["runEventLifecycleState"] = "drafted"
    expect_run_event_kernel_validation_failure(
        "run_event_item_lifecycle_drafted",
        tampered,
        "runEventLifecycleState must equal emitted",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItems"][0]["requiredFlag"] = True
    expect_run_event_kernel_validation_failure(
        "run_event_item_required_flag_true",
        tampered,
        "requiredFlag must be false",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItems"][0]["verifierVerdictOwner"] = "creator_agent"
    expect_run_event_kernel_validation_failure(
        "run_event_item_creator_owns_verdict",
        tampered,
        "verifierVerdictOwner must equal verifier-runtime-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItems"][0]["permissionFlagOverrides"]["externalSideEffectsAllowed"] = True
    expect_run_event_kernel_validation_failure(
        "run_event_item_permission_override_true",
        tampered,
        "permissionFlagOverrides.externalSideEffectsAllowed must be false",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventItems"][0]["sourceEventRef"] = "unknown.event"
    expect_run_event_kernel_validation_failure(
        "run_event_item_source_event_ref_drift",
        tampered,
        "sourceEventRef must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["sourceEventCoverage"]["everyEventTypeDomainCovered"] = False
    expect_run_event_kernel_validation_failure(
        "source_event_coverage_every_domain_false",
        tampered,
        "everyEventTypeDomainCovered must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["sourceEventCoverage"]["everyRunEventItemReferencesValidEventType"] = False
    expect_run_event_kernel_validation_failure(
        "source_event_coverage_every_ref_false",
        tampered,
        "everyRunEventItemReferencesValidEventType must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["sourceEventCoverage"]["everyLogEventTypeResolved"] = False
    expect_run_event_kernel_validation_failure(
        "source_event_coverage_every_log_false",
        tampered,
        "everyLogEventTypeResolved must be true",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["sourceEventCoverage"]["eventTypeDomainKeys"] = ["unknown.event"]
    expect_run_event_kernel_validation_failure(
        "source_event_coverage_keys_drift",
        tampered,
        "eventTypeDomainKeys must equal sorted RunEventLogV1.eventTypeDomain",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["sourceEventCoverage"]["uniqueRunEventItemRefs"] = ["run.created"]
    expect_run_event_kernel_validation_failure(
        "source_event_coverage_unique_refs_drift",
        tampered,
        "uniqueRunEventItemRefs must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["promotionConditions"]["requiredVerdict"] = "needs_human_check"
    expect_run_event_kernel_validation_failure(
        "promotion_required_verdict_drift",
        tampered,
        "promotionConditions.requiredVerdict must be passed",
    )
    ...
    tampered["promotionConditions"]["promoted"] = True
    expect_run_event_kernel_validation_failure(
        "promotion_promoted_true_without_gates",
        tampered,
        "promotionConditions.promoted must be true only when",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["promotionConditions"]["verdictOwner"] = "creator_agent"
    expect_run_event_kernel_validation_failure(
        "promotion_verdict_owner_drift",
        tampered,
        "promotionConditions.verdictOwner must be verifier-runtime-v1",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["taskSpecBinding"]["taskSpecSchemaVersion"] = "regression-task-spec-v1"
    expect_run_event_kernel_validation_failure(
        "task_spec_binding_regression_schema",
        tampered,
        "taskSpecSchemaVersion must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["taskSpecBinding"]["taskSpecWorkloadType"] = "read_only_regression_evidence_demo"
    expect_run_event_kernel_validation_failure(
        "task_spec_binding_regression_workload",
        tampered,
        "taskSpecWorkloadType must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runBinding"]["runState"] = "failed"
    expect_run_event_kernel_validation_failure(
        "run_binding_run_state_drift",
        tampered,
        "runBinding.runState must equal RunV1.runState",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "step_list_binding_count_drift",
        tampered,
        "stepListBinding.stepCount must equal",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "tool_call_list_binding_count_drift",
        tampered,
        "toolCallListBinding.toolCallCount must equal",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "capability_kernel_binding_count_drift",
        tampered,
        "capabilityKernelBinding.capabilityCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventLogBinding"]["eventCount"] = 99
    expect_run_event_kernel_validation_failure(
        "run_event_log_binding_event_count_drift",
        tampered,
        "runEventLogBinding.eventCount must equal",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "run_event_log_binding_terminal_state_drift",
        tampered,
        "runEventLogBinding.terminalState must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["runEventLogBinding"]["eventTypeDomain"] = ["run.created"]
    expect_run_event_kernel_validation_failure(
        "run_event_log_binding_event_type_domain_drift",
        tampered,
        "runEventLogBinding.eventTypeDomain must equal",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "observation_list_binding_count_drift",
        tampered,
        "observationListBinding.observationItemCount must equal",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "evidence_list_binding_count_drift",
        tampered,
        "evidenceListBinding.evidenceItemCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["verifierResultBinding"]["verdict"] = "passed"
    expect_run_event_kernel_validation_failure(
        "verifier_result_binding_verdict_drift",
        tampered,
        "verifierResultBinding.verdict must equal VerifierResultV1.verdict",
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["policyManifestBinding"]["unlocked"] = True
    expect_run_event_kernel_validation_failure(
        "policy_manifest_binding_unlocked_drift",
        tampered,
        "policyManifestBinding.unlocked must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["policyKernelBinding"]["policyCount"] = 99
    expect_run_event_kernel_validation_failure(
        "policy_kernel_binding_policy_count_drift",
        tampered,
        "policyKernelBinding.policyCount must equal",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["policyKernelBinding"]["policyKernelSchemaVersion"] = "policy-manifest-v1"
    expect_run_event_kernel_validation_failure(
        "policy_kernel_binding_schema_drift",
        tampered,
        "policyKernelBinding.policyKernelSchemaVersion must equal",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "approval_decision_binding_granted_drift",
        tampered,
        "approvalDecisionBinding.approvalGranted must equal",
    )
    ...
    expect_run_event_kernel_validation_failure(
        "reuses_regression_run_event_artifact",
        tampered,
        "must not reuse any first-workload regression run event artifact",
    )

    tampered = json.loads(json.dumps(artifact))
    tampered["regressionWorkloadIsolation"]["reusesRegressionResultArtifact"] = True
    expect_run_event_kernel_validation_failure(
        "reuses_regression_result_artifact",
        tampered,
        "must not reuse RegressionResultArtifactV1",
    )
    ...
    tampered = json.loads(json.dumps(artifact))
    tampered["coordinationBinding"]["delegationManifestRef"] = "artifacts/coordination/other.json"
    expect_run_event_kernel_validation_failure(
        "coordination_binding_delegation_drift",
        tampered,
        "coordinationBinding.delegationManifestRef must equal",
    )
    ...
    for hash_role, label in [
        ...
        ("test_execution_delivery_manifest", "source_hash_mismatch_delivery_manifest"),
        ("test_execution_policy_manifest", "source_hash_mismatch_policy_manifest"),
        ("test_execution_policy_kernel", "source_hash_mismatch_policy_kernel"),
        ("test_execution_run_artifact", "source_hash_mismatch_run_artifact"),
        ("test_execution_step_list", "source_hash_mismatch_step_list"),
        ("test_execution_tool_call_list", "source_hash_mismatch_tool_call_list"),
        ("human_approval_decision", "source_hash_mismatch_human_approval_decision"),
...
def run_run_event_kernel_builder(artifact_out: Path) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts/run_event_kernel.py"),
        "--out",
        str(artifact_out),
    ]
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=False)
...
def main() -> None:
    ...
    validate_committed_run_artifact()
    validate_committed_step_list_artifact()
    ...
    validate_committed_run_event_kernel_artifact()
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        generated_evidence_list_artifact = temp_root / "post_mvp_test_execution_evidence_list.json"
        run_evidence_list_artifact_builder(generated_evidence_list_artifact)
        ...
        generated_run_artifact = temp_root / "post_mvp_test_execution_run.json"
        run_run_artifact_builder(generated_run_artifact)
        if load_json(generated_run_artifact) != load_json(RUN_ARTIFACT_PATH):
            raise AssertionError(
                "Committed RunV1 artifact differs from deterministic CLI output"
            )
        generated_step_list_artifact = temp_root / "post_mvp_test_execution_step_list.json"
        run_step_list_artifact_builder(generated_step_list_artifact)
        if load_json(generated_step_list_artifact) != load_json(STEP_LIST_ARTIFACT_PATH):
            raise AssertionError(
                "Committed StepListV1 artifact differs from deterministic CLI output"
            )
        generated_tool_call_list_artifact = temp_root / "post_mvp_test_execution_tool_call_list.json"
        run_tool_call_list_artifact_builder(generated_tool_call_list_artifact)
        if load_json(generated_tool_call_list_artifact) != load_json(TOOL_CALL_LIST_ARTIFACT_PATH):
            raise AssertionError(
                "Committed ToolCallListV1 artifact differs from deterministic CLI output"
            )
        generated_intent_artifact = temp_root / "post_mvp_test_execution_intent.json"
        run_intent_artifact_builder(generated_intent_artifact)
        if load_json(generated_intent_artifact) != load_json(INTENT_ARTIFACT_PATH):
            raise AssertionError(
                "Committed IntentV1 artifact differs from deterministic CLI output"
            )
        generated_capability_kernel_artifact = temp_root / "post_mvp_capability_v1_catalog.json"
        run_capability_kernel_builder(generated_capability_kernel_artifact)
        ...
        generated_run_event_kernel_artifact = temp_root / "post_mvp_run_event_v1_catalog.json"
        run_run_event_kernel_builder(generated_run_event_kernel_artifact)
        if load_json(generated_run_event_kernel_artifact) != load_json(RUN_EVENT_KERNEL_ARTIFACT_PATH):
            raise AssertionError(
                "Committed RunEventV1 artifact differs from deterministic CLI output"
            )
