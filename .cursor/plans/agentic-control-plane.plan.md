...
### 2026-05-14 08:20 UTC Automation Sprint：TestExecutionTaskSpecV1 Second Workload Envelope Gate

Active phase：post-MVP Kernel Generalization / Multi-Workload Expansion（Phase 1a-9 contract-only MVP 已满足，Agent Coordination Layer 五件套 contract 完成，正在打开第二 workload）。

Selected slice：

```text
Add TestExecutionTaskSpecV1 envelope contract artifact at artifacts/task_specs/post_mvp_test_execution_task_spec.json so /usr/bin/python3 scripts/validate_repo.py verifies a workload-independent TaskSpecV1 subtype for Test Execution / Failure Triage, bound to DelegationManifestV1 / CreatorVerifierPairingV1 / AgentMessageManifestV1 / NegotiationRecordV1 / BroadcastSubscriptionManifestV1 by ref and source content hash, with regression/email TaskSpec fields, regression_result, email_draft, send_email and regression-task-spec-v1 explicitly forbidden.
```

为什么这是下一步：

- 上一轮已 land BroadcastSubscriptionManifestV1，Agent Coordination Layer 五件套 contract（delegation / creator-verifier / direct communication / negotiation / broadcast）全部完成；EvaluationReportV1 的 `nextRecommendedSlice` 已经明确推荐打开第二 workload 的 `TestExecutionTaskSpecV1` envelope。
- Post-MVP slice priority #1（generalize workload-independent kernel primitive：`TaskSpecV1`）与 #2（add second workload：`Test Execution / Failure Triage`）正好叠加在同一切片：把 `TaskSpecV1` envelope 从单一 `regression-task-spec-v1` 推广为多 subtype，并通过第二 workload 验证。
- `.github/workflows/validate.yml` 和 `.github/workflows/auto-merge-cursor-pr.yml` 均存在；本轮 baseline `/usr/bin/python3 scripts/validate_repo.py` 已通过。

OS kernel primitive advanced：

- 在 `TaskSpecV1` envelope 上引入第二个具体 subtype `test-execution-task-spec-v1`，与既有的 `regression-task-spec-v1` 并列。`taskSpecEnvelopeSchemaVersion` 字段使 envelope 在 schema 层显式存在，未来 `CodePatchTaskSpecV1`、`PRReviewTaskSpecV1` 等可以复用同一 envelope。
- `TaskSpecV1` envelope 抽象出 workload-independent 字段：`workloadType` / `workloadCategory` / `intent` / `goal` / `inputs[]` / `allowedCapabilityClasses` / `forbiddenEffectClasses` / `requiredEvidence` / `expectedArtifactKinds` / `successCriteria` / `approvalPoints` / `verifierExpectations` / `policyBoundary` / `coordinationBinding` / `regressionWorkloadIsolation` / `nonRegressionReusePath` / `sourceArtifacts` / `invariants`，不再依赖 regression-only 的 `inputLogPath` / `regression_result.json` / `email_draft.md` / `send_email`。

Non-regression workload reuse path：

- `Test Execution / Failure Triage`：本 artifact 直接覆盖；`inputs[]` 引入 `TestExecutionResultV1` 和可选 `DiagnosticBundleV1`，`expectedArtifactKinds` 引入 `TestExecutionResultV1` / `FailureTriageReportV1` / `EvidenceListV1`。
- `Code Patch / Review Loop`：未来 `CodePatchTaskSpecV1` 可复用相同 envelope 结构、相同 5 件套 coordination binding 和相同 `regressionWorkloadIsolation` 守护。
- `PR Review`：未来 `PRReviewTaskSpecV1` 同理；本 artifact 的 `nonRegressionReusePath` 字段已声明 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review` 三条路径。

Coordination protocol / invariant clarified：

- `coordinationBinding` 字段强制 TaskSpecV1 subtype 绑定 Agent Coordination Layer 五件套 contract artifact，并通过 `sourceArtifacts[].contentHash` 把上游 coordination contract 当作不可变前提；任何一个上游 contract 漂移都会让本 artifact validate 失败。
- `verifierExpectations.verdictOwner = verifier-runtime-v1` 与 `creatorMustNotOwnVerdict = true` 把 creator-verifier 不变量延伸到第二 workload；`forbiddenEffectClasses` 用 workload-independent 类（`send_external_communication`、`trigger_release_or_deployment`、`call_gui_desktop_cua_or_browser_capability` 等）取代 regression-only 的 `send_email`。
- `policyBoundary` 对 `externalSideEffectsAllowed` / `repositoryMutationAllowed` / `externalCommunicationAllowed` / `releaseOrDeploymentAllowed` / `guiOrBrowserOrDesktopCallAllowed` / `publicAccessAllowed` 全部强制为 `false`，与既有 BroadcastSubscriptionManifestV1 / NegotiationRecordV1 / DelegationManifestV1 的 policy invariant 对齐。

为什么这不是另一个 regression/email fixture：

- artifact 路径为 `artifacts/task_specs/post_mvp_test_execution_task_spec.json`，scope 为 `task_spec_envelope`，不是 regression result、email draft、approval lifecycle、delivery unlock 或 policy unlock。
- `workloadType` 必须等于 `test_execution_failure_triage`，validator 显式拒绝把它改成 `read_only_regression_evidence_demo` 或任何 regression workload。
- `taskSpecEnvelopeSchemaVersion` 必须为 `task-spec-v1`，`schemaVersion` 必须为 `test-execution-task-spec-v1`；validator 拒绝把 envelope 退回 `regression-task-spec-v1`。
- `regressionWorkloadIsolation.forbiddenFields` 包含 `regression_result` / `regression_result.json` / `email_draft` / `email_draft.md` / `send_email` / `regression-task-spec-v1`；defensive scan 拒绝这些 token 在 forbiddenFields 之外再次出现。
...
- `scripts/test_execution_task_spec.py` 提供 `build_test_execution_task_spec` / `validate_test_execution_task_spec` 与 CLI（`--out` / `--validate`）。
- 新增 committed `artifacts/task_specs/post_mvp_test_execution_task_spec.json`，schemaVersion 为 `test-execution-task-spec-v1`，taskSpecEnvelopeSchemaVersion 为 `task-spec-v1`。
- `scripts/validate_repo.py` 验证 committed artifact、deterministic builder output 与 forced-failure cases：workload_type_regression_reuse / envelope_schema_regression_reuse / regression_field_reuse_allowed / regression_schema_reuse_allowed / expected_artifact_kinds_regression_overlap / forbidden_effects_drop_communication / policy_external_side_effects_allowed / policy_repository_mutation_allowed / verifier_owner_override / creator_owns_verdict_allowed / coordination_binding_missing_broadcast / coordination_binding_wrong_path / non_regression_reuse_path_missing / source_hash_mismatch_broadcast。
- `EvaluationReportV1` 把 TestExecutionTaskSpecV1 artifact 纳入 source hash，并把下一推荐切片推进为 workload-independent `TestExecutionResultV1` ArtifactV1 subtype。

实现摘要：

- 新增 `scripts/test_execution_task_spec.py`，包含 schema 常量、builder、validator、deterministic CLI（`--out` / `--validate`）和五件套 coordination contract 绑定逻辑。
- 新增 committed `artifacts/task_specs/post_mvp_test_execution_task_spec.json`。
- 更新 `scripts/validate_repo.py`：REQUIRED_FILES 增加 `scripts/test_execution_task_spec.py` 与 `artifacts/task_specs/post_mvp_test_execution_task_spec.json`，PLAN markers 增加 `TestExecutionTaskSpecV1` / `test-execution-task-spec-v1` / `test_execution_failure_triage` / `post_mvp_test_execution_task_spec.json`，新增 committed artifact 校验、deterministic CLI replay 与 14 条 forced-failure case。
- 更新 `scripts/evaluation_report.py` 与 `artifacts/evaluation/phase9_mvp_evaluation_report.json`：把 TestExecutionTaskSpecV1 加入 source hash，并把 `nextRecommendedSlice` 推进为 workload-independent `TestExecutionResultV1` ArtifactV1 subtype。

Build vs Integrate：

- Build：`TestExecutionTaskSpecV1` schema、deterministic builder / validator / CLI、committed artifact、14 条 forced-failure case、`EvaluationReportV1` source hash 集成。
- Integrate later：真实 test framework（pytest / jest / go test / cargo test 等）驱动、真实 TestExecutionResultV1 / DiagnosticBundleV1 fixture、failure triage rule engine、与 IDE / CI / sandbox 的真实接入、跨 workload TaskSpec orchestrator。
...
- Baseline /usr/bin/python3 scripts/validate_repo.py before edits：通过。
- /usr/bin/python3 -m py_compile scripts/*.py：通过。
- /usr/bin/python3 scripts/test_execution_task_spec.py --out artifacts/task_specs/post_mvp_test_execution_task_spec.json：通过。
- /usr/bin/python3 scripts/test_execution_task_spec.py --validate artifacts/task_specs/post_mvp_test_execution_task_spec.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --report-out artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --validate-report artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/validate_repo.py：通过，覆盖 TestExecutionTaskSpecV1 committed artifact、deterministic builder output 与 14 条 forced-failure case。
- git diff --check：通过。
```

剩余风险：

- `TestExecutionTaskSpecV1` 仍是 static envelope contract，不驱动任何真实 test framework；下一切片必须给出 workload-independent `TestExecutionResultV1` ArtifactV1 subtype，才能形成 (TaskSpec, Artifact, Evidence, Verifier) 闭环。
- 当前 envelope 只示范第二 workload；`Code Patch / Review Loop`、`PR Review` 等 TaskSpec subtype 仍未实现。
- 未来需要把 `regression-task-spec-v1` 与 `test-execution-task-spec-v1` 抽出共同 base schema（真正的 `task-spec-v1` envelope），现在只用 `taskSpecEnvelopeSchemaVersion` 字段声明它们同属一个 envelope。

下一轮建议：

```text
新增 workload-independent TestExecutionResultV1 ArtifactV1 subtype contract artifact：
...
### 2026-05-14 09:00 UTC Automation Sprint：FailureTriageReportV1 Second ArtifactV1 Subtype Gate

Active phase：post-MVP Kernel Generalization / Multi-Workload Expansion（Phase 1a-9 contract-only MVP 已满足，Agent Coordination Layer 五件套 contract 完成，第二 workload 的 TaskSpec envelope 与 TestExecutionResultV1 已 land，正在闭合第二 workload 的下游 triage ArtifactV1 子类型）。

Selected slice：

```text
Add FailureTriageReportV1 ArtifactV1 subtype contract artifact at artifacts/test_execution/post_mvp_failure_triage_report.json so /usr/bin/python3 scripts/validate_repo.py verifies a workload-independent triage ArtifactV1 subtype for Test Execution / Failure Triage, bound to TestExecutionResultV1 (test-execution-result-v1) and TestExecutionTaskSpecV1 (test-execution-task-spec-v1) by content hash, and to DelegationManifestV1 / CreatorVerifierPairingV1 / AgentMessageManifestV1 / NegotiationRecordV1 / BroadcastSubscriptionManifestV1 by ref and source content hash, with regression_result / email_draft / send_email / regression-task-spec-v1 / regression-result-artifact-v1 explicitly forbidden.
```

为什么这是下一步：

- 上一轮 `EvaluationReportV1.nextRecommendedSlice` 明确要求新增 workload-independent `FailureTriageReportV1` ArtifactV1 subtype，把 (TaskSpecV1, ArtifactV1, EvidenceListV1, VerifierResultV1) 的 OS kernel 闭环在第二 workload 上从「测试执行结果」推进到「失败归因」。
- Post-MVP slice priority #1（generalize workload-independent kernel primitive：`ArtifactV1`）与 #2（add second workload：`Test Execution / Failure Triage`）继续叠加：在 `ArtifactV1` envelope 上引入第三个具体 subtype `failure-triage-report-v1`，与 `regression-result-artifact-v1`、`test-execution-result-v1` 并列，证明 envelope 可以承载多份 creator output。
...
- 在 `ArtifactV1` envelope 上引入第三个 subtype `failure-triage-report-v1`；`artifactEnvelopeSchemaVersion=artifact-v1` 显式声明 envelope 复用。未来 `PatchArtifactV1`、`ReviewFindingArtifactV1` 可以复用同一 envelope。
- `EvidenceListV1` 复用：FailureTriageReportV1 自身不引入新的 evidence，只引用 TestExecutionResultV1.evidenceList 中已存在的 evidence id，并校验 evidence.caseId 与 finding.caseRef 一致；这强制 creator 不能凭空捏造 evidence。
- `TaskSpec → Artifact → Artifact` 链：`testExecutionResultBinding` 与 `taskSpecBinding` 同时绑定上游 TaskSpec envelope 和上游 Artifact envelope，证明 OS kernel 可以表达「一个 creator artifact 引用另一个 creator artifact」的因果链。
- `Verifier` ownership invariant：`verifierVerdictOwner=verifier-runtime-v1` 与 `creatorMustNotOwnVerdict=true` 把 creator-verifier 不变量延伸到第二个 ArtifactV1 subtype；triage artifact 只声明 hypothesis，不声明 acceptance。

Non-regression workload reuse path：

- `Test Execution / Failure Triage`：本 artifact 直接覆盖；finding / refinedFailureClassification / rootCauseHypothesis / remediationCategory / supportingEvidenceIds 字段均 workload-independent。
- `Code Patch / Review Loop`：未来 `ReviewFindingArtifactV1` 可复用 finding/observation/evidence/remediation 四元组结构；review hunk 替换 test case，review evidence 替换 test evidence。
- `PR Review`：未来 `PRReviewFindingArtifactV1` 同理；本 artifact 的 `nonRegressionReusePath` 字段已声明 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review` 三条路径。

Coordination protocol / invariant clarified：

- `creatorAgent=creator_agent` 与 `verifierVerdictOwner=verifier-runtime-v1` 保留 CreatorVerifierPairingV1 不变量：creator output 不能自证 acceptance。
- `coordinationBinding` 字段强制 ArtifactV1 子类型绑定 Agent Coordination Layer 五件套 contract artifact，并通过 `sourceArtifacts[].contentHash` 把上游 coordination / task spec / test execution result contract 当作不可变前提；任何一个上游 contract 漂移都会让本 artifact validate 失败。
- 每个 finding 的 `mayBroadcastDownstream=false` 把 broadcast invariant 钉死在 BroadcastSubscriptionManifestV1，triage artifact 不允许自带 fan-out 通道。
- `policyBoundary` 对 `externalSideEffectsAllowed` / `repositoryMutationAllowed` / `externalCommunicationAllowed` / `releaseOrDeploymentAllowed` / `guiOrBrowserOrDesktopCallAllowed` / `publicAccessAllowed` 全部强制为 `false`，与既有 BroadcastSubscriptionManifestV1 / NegotiationRecordV1 / DelegationManifestV1 / TestExecutionTaskSpecV1 / TestExecutionResultV1 的 policy invariant 对齐。

为什么这不是另一个 regression/email fixture：

- artifact 路径为 `artifacts/test_execution/post_mvp_failure_triage_report.json`，scope 为 `test_execution_failure_triage_artifact_contract`，不是 regression result、email draft、approval lifecycle、delivery unlock 或 policy unlock。
- `workloadType` 必须等于 `test_execution_failure_triage`，validator 显式拒绝把它改成 `read_only_regression_evidence_demo` 或任何 regression workload；`kind` 必须为 `FailureTriageReportV1`，`schemaVersion` 必须为 `failure-triage-report-v1`。
- `regressionWorkloadIsolation.forbiddenFields` 包含 `regression_result` / `regression_result.json` / `email_draft` / `email_draft.md` / `send_email` / `regression-task-spec-v1` / `regression-result-artifact-v1`；defensive scan 拒绝这些 token 在 forbiddenFields 之外再次出现。`reusesTestExecutionResultArtifactSchema=false` 把 envelope 复用与 schema 复用区分开。
...
- `scripts/failure_triage_report.py` 提供 `build_failure_triage_report` / `validate_failure_triage_report` 与 CLI（`--out` / `--validate`）。
- 新增 committed `artifacts/test_execution/post_mvp_failure_triage_report.json`，schemaVersion 为 `failure-triage-report-v1`，artifactEnvelopeSchemaVersion 为 `artifact-v1`。
- `scripts/validate_repo.py` 验证 committed artifact、deterministic builder output 与多条 forced-failure case：workload_type_regression_reuse / kind_reuses_test_execution_result / schema_regression_reuse / envelope_schema_regression_reuse / creator_owns_verdict / creator_must_not_own_verdict_disabled / task_spec_binding_regression_schema_reuse / test_execution_result_binding_regression_schema_reuse / test_execution_result_binding_hash_drift / policy_external_side_effects_allowed / policy_external_communication_allowed / triage_findings_empty / triages_passed_case / finding_observed_status_drift / finding_evidence_belongs_to_other_case / finding_missing_evidence / finding_refined_classification_invalid / finding_remediation_invalid / finding_broadcast_downstream_allowed / summary_count_drift / coordination_binding_missing_broadcast / coordination_binding_wrong_path / non_regression_reuse_path_missing / reuses_test_execution_result_schema / source_hash_mismatch_test_execution_result / source_hash_mismatch_broadcast。
- `EvaluationReportV1` 把 FailureTriageReportV1 artifact 纳入 source hash，并把下一推荐切片推进为 workload-independent `VerifierResultV1` ArtifactV1 subtype。

实现摘要：

- 新增 `scripts/failure_triage_report.py`，包含 schema 常量、builder、validator、deterministic CLI（`--out` / `--validate`）和 ArtifactV1 envelope / TaskSpec & TestExecutionResult dual binding / 五件套 coordination binding 逻辑。
- 新增 committed `artifacts/test_execution/post_mvp_failure_triage_report.json`。
- 更新 `scripts/validate_repo.py`：REQUIRED_FILES 增加 `scripts/failure_triage_report.py` 与 `artifacts/test_execution/post_mvp_failure_triage_report.json`，PLAN markers 增加 `FailureTriageReportV1` / `failure-triage-report-v1` / `post_mvp_failure_triage_report.json`，新增 committed artifact 校验、deterministic CLI replay 与 26 条 forced-failure case。
- 更新 `scripts/evaluation_report.py` 与 `artifacts/evaluation/phase9_mvp_evaluation_report.json`：把 FailureTriageReportV1 加入 source hash，并把 `nextRecommendedSlice` 推进为 workload-independent `VerifierResultV1` ArtifactV1 subtype。

Build vs Integrate：

- Build：`FailureTriageReportV1` schema、deterministic builder / validator / CLI、committed artifact、26 条 forced-failure case、`EvaluationReportV1` source hash 集成。
- Integrate later：真实失败归因 rule engine、根因聚类 / LLM 解释器、跨多 framework triage（pytest / jest / go test / cargo test）、与 IDE / PR review 工具的接入、ArtifactV1 envelope 抽取为独立 schema 文件、跨 workload triage catalogue。
...
- Baseline /usr/bin/python3 scripts/validate_repo.py before edits：通过。
- /usr/bin/python3 -m py_compile scripts/*.py：通过。
- /usr/bin/python3 scripts/failure_triage_report.py --out artifacts/test_execution/post_mvp_failure_triage_report.json：通过。
- /usr/bin/python3 scripts/failure_triage_report.py --validate artifacts/test_execution/post_mvp_failure_triage_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --report-out artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --validate-report artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/validate_repo.py：通过，覆盖 FailureTriageReportV1 committed artifact、deterministic builder output 与 26 条 forced-failure case。
- git diff --check：通过。
```

剩余风险：

- `FailureTriageReportV1` 仍是 static contract artifact，不驱动任何真实 root-cause analysis；下一切片应给出 workload-independent `VerifierResultV1` ArtifactV1 subtype，把 (TaskSpec, Artifact, Evidence, Verifier) 闭环在第二 workload 上真正闭合 verifier verdict 这一端。
- 当前 artifact 复用 first workload 的 5 件套 coordination contract artifact 作为 source fixture；`Code Patch / Review Loop` 与 `PR Review` 的 TaskSpec / Artifact subtype 仍未实现。
- `ArtifactV1` envelope 目前仍通过 `artifactEnvelopeSchemaVersion` 字段表达；未来需要把 `regression-result-artifact-v1` / `test-execution-result-v1` / `failure-triage-report-v1` 抽出共同 base schema（真正的 `artifact-v1` envelope）。

下一轮建议：

```text
新增 workload-independent VerifierResultV1 ArtifactV1 subtype contract artifact：
...
### 2026-05-14 10:20 UTC Automation Sprint：DeliveryManifestV1 Workload-Independent Delivery Primitive Gate

Active phase：post-MVP Kernel Generalization / Multi-Workload Expansion（Phase 1a-9 contract-only MVP 已满足，Agent Coordination Layer 五件套 contract 完成，第二 workload 的 TaskSpec envelope、TestExecutionResultV1、FailureTriageReportV1、VerifierResultV1、CapabilityManifestV1、RunEventLogV1 均已 land，正在闭合 OS kernel 的 Delivery 原语，把「交付」从 first-workload regression `email_draft.md` / phase9 delivery report 抽象为 workload-independent ArtifactV1 子类型）。

Selected slice：

```text
Add DeliveryManifestV1 ArtifactV1 subtype contract artifact at artifacts/delivery/post_mvp_test_execution_delivery_manifest.json so /usr/bin/python3 scripts/validate_repo.py verifies a workload-independent Delivery primitive for Test Execution / Failure Triage, bound to TestExecutionTaskSpecV1 (test-execution-task-spec-v1), CapabilityManifestV1 (capability-manifest-v1), VerifierResultV1 (verifier-result-v1), RunEventLogV1 (run-event-log-v1), and HumanApprovalDecisionV1 (human-approval-decision-v1) by content hash, and to DelegationManifestV1 / CreatorVerifierPairingV1 / AgentMessageManifestV1 / NegotiationRecordV1 / BroadcastSubscriptionManifestV1 by ref and source content hash, with regression_result / email_draft / send_email / regression-task-spec-v1 / regression-result-artifact-v1 explicitly forbidden, channels drawn from a workload-independent kind domain (internal_run_summary_card / verifier_verdict_record / human_review_queue_entry / triage_artifact_handoff / internal_event_log_archive), channelStatus drawn from a closed domain that excludes any sent / delivered / published terminal status, requiredVerdictForUnlock=passed, requiredTerminalRunState=completed, per-channel policyBoundary forcing externalSideEffectsAllowed=false / externalCommunicationAllowed=false / repositoryMutationAllowed=false / releaseOrDeploymentAllowed=false / guiOrBrowserOrDesktopCallAllowed=false / publicAccessAllowed=false, and verifierVerdictOwner=verifier-runtime-v1.
```

为什么这是下一步：

- 上一轮 `EvaluationReportV1.nextRecommendedSlice` 明确要求新增 workload-independent `DeliveryV1 / DeliveryManifestV1` ArtifactV1 subtype，把 OS kernel 的交付原语在第二 workload 上独立抽象出来。
- Post-MVP slice priority #1（generalize workload-independent kernel primitive：`DeliveryV1`）与 #2（add second workload：`Test Execution / Failure Triage`）继续叠加：在 `ArtifactV1` envelope 上引入第七个具体 subtype `delivery-manifest-v1`，与 `regression-result-artifact-v1`、`test-execution-result-v1`、`failure-triage-report-v1`、`verifier-result-v1`、`capability-manifest-v1`、`run-event-log-v1` 并列。
- 这是 OS kernel 的 `Delivery` 抽象第一次以 workload-independent 形式独立成 ArtifactV1 子类型。既有的 `phase9_delivery_report.json` 与 `phase9_multi_agent_delivery_manifest.json` 是 first-workload regression demo bound 的 `multi-agent-delivery-manifest-v1` / `delivery-report-v1` 旧 schema；新 manifest 在显式 `regressionWorkloadIsolation.reusesPhase9DeliveryReport=false` 的前提下，用新的 `delivery-manifest-v1` envelope 和 `delivery-channel-v1` channel schema 描述第二 workload 的 channel-agnostic 交付集合。
- `.github/workflows/validate.yml` 与 `.github/workflows/auto-merge-cursor-pr.yml` 均存在；本轮 baseline `/usr/bin/python3 scripts/validate_repo.py` 已通过。

OS kernel primitive advanced：

- 在 `ArtifactV1` envelope 上引入第七个 subtype `delivery-manifest-v1`；`artifactEnvelopeSchemaVersion=artifact-v1` 显式声明 envelope 复用。未来 `CodePatchDeliveryManifestV1`、`PRReviewDeliveryManifestV1`、`DocumentationUpdateDeliveryManifestV1`、`IncidentAnalysisDeliveryManifestV1` 等可以复用同一 envelope。
- `DeliveryV1` / `delivery-channel-v1`：第一次把 OS kernel 中的 `Delivery` 抽象作为独立 schema 落地，每个 channel 的字段为 `schemaVersion / channelName / channelKind / channelStatus / unlocked / approvalRef / evidenceContract / payloadContract / policyBoundary`，全部 workload-independent。
- `Verifier ↔ RunEventLog ↔ Approval ↔ Delivery` 一致性：`verdictGate` 把 `VerifierResultV1.verdict` 钉到 `requiredVerdictForUnlock=passed`；`runTerminalStateGate` 把 `RunEventLogV1.terminalState` 钉到 `requiredTerminalState=completed`；`approvalGate` 把 `HumanApprovalDecisionV1.decision.approvalGranted` 钉到 `requiredDecision=approved`。任何 channel 只有当三个 gate 全部 `unlocked` 时才允许 `channelStatus=drafted_internal_only / unlocked=true`；否则 channel 必须等于 gate-derived blocked 状态（`blocked_by_verdict` / `blocked_by_policy` / `blocked_by_approval`）。OS kernel 第一次拥有机器可验证的 Delivery 三重门约束。
- `Policy ↔ Delivery` 一致性：manifest 自身与每个 channel 的 6 个 policy flag 全部 `false`；validator 显式拒绝任何 `sent` / `delivered` / `published` 终态 channelStatus。

Non-regression workload reuse path：

- `Test Execution / Failure Triage`：本 artifact 直接覆盖；5 个 channel 全部使用 workload-independent kind / status。
- `Code Patch / Review Loop`：未来 `CodePatchDeliveryManifestV1` 可复用同一 `delivery-manifest-v1` envelope、`delivery-channel-v1` channel schema、verdictGate / runTerminalStateGate / approvalGate 三重门；`channels` 切换到 `code_patch_diff_summary` / `code_review_handoff_queue_entry` 等仍然 workload-independent 的 kind。
- `PR Review` / `Documentation Update` / `Incident / Log Analysis`：同理。本 artifact 的 `nonRegressionReusePath` 字段已声明 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review` 三条路径。

Coordination protocol / invariant clarified：

- `coordinationBinding` 字段强制 DeliveryManifestV1 绑定 Agent Coordination Layer 五件套 contract artifact，并通过 `sourceArtifacts[].contentHash` 把上游 coordination / task spec / capability manifest / verifier result / run event log / approval decision contract 当作不可变前提；任何一个上游 contract 漂移都会让本 artifact validate 失败。
- `verifierVerdictOwner=verifier-runtime-v1` 与 `verifierExpectations.creatorMustNotOwnVerdict=true` 把 CreatorVerifierPairingV1 不变量延伸到交付层：creator 写入 channel 草稿，但 verifier 拥有 acceptance；channel 的 `unlocked=true` 不能由 creator 单方面声明。
- `policyBoundary` 对 `externalSideEffectsAllowed` / `repositoryMutationAllowed` / `externalCommunicationAllowed` / `releaseOrDeploymentAllowed` / `guiOrBrowserOrDesktopCallAllowed` / `publicAccessAllowed` 全部强制为 `false`，与既有 BroadcastSubscriptionManifestV1 / NegotiationRecordV1 / DelegationManifestV1 / TestExecutionTaskSpecV1 / TestExecutionResultV1 / FailureTriageReportV1 / VerifierResultV1 / CapabilityManifestV1 / RunEventLogV1 的 policy invariant 对齐：交付原语自身不允许触发 side effect / external communication / public access。

为什么这不是另一个 regression/email fixture：

- artifact 路径为 `artifacts/delivery/post_mvp_test_execution_delivery_manifest.json`（独立于 `artifacts/delivery/phase9_delivery_report.json` 与 `artifacts/delivery/phase9_multi_agent_delivery_manifest.json` 这两个 first-workload regression demo artifact），scope 为 `test_execution_failure_triage_delivery_manifest_contract`，不是 regression result、email draft、approval lifecycle、delivery unlock 或 policy unlock。
- `workloadType` 必须等于 `test_execution_failure_triage`，validator 显式拒绝把它改成 `read_only_regression_evidence_demo` 或任何 regression workload；`kind` 必须为 `DeliveryManifestV1`，`schemaVersion` 必须为 `delivery-manifest-v1`，每个 channel 的 `schemaVersion` 必须为 `delivery-channel-v1`。
- `regressionWorkloadIsolation.forbiddenFields` 包含 `regression_result` / `regression_result.json` / `email_draft` / `email_draft.md` / `send_email` / `regression-task-spec-v1` / `regression-result-artifact-v1`；新增 `reusesRegressionEmailDraft=false` / `reusesPhase9DeliveryReport=false` / `reusesSendEmailCapability=false` 把本 artifact 与 first-workload regression delivery artifact 显式隔离。defensive scan 拒绝这些 token 在 forbiddenFields 之外再次出现。
- `channelKindDomain` 不包含 `email` / `pr_comment` / `slack` / `webhook` / `external_*` 等任何外部 channel；`channelStatusDomain` 不包含 `sent` / `delivered` / `published` / `dispatched` 等任何 real-delivery 终态。所有 channel 永远只能停留在 `drafted_internal_only` / `pending_*` / `blocked_*`。
- `nonRegressionReusePath` 必须包含 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review`，强制本 artifact 至少有两个 non-regression 复用路径。

Acceptance criteria：

- `scripts/delivery_manifest.py` 提供 `build_delivery_manifest` / `validate_delivery_manifest` 与 CLI（`--out` / `--validate`）。
- 新增 committed `artifacts/delivery/post_mvp_test_execution_delivery_manifest.json`，schemaVersion 为 `delivery-manifest-v1`，artifactEnvelopeSchemaVersion 为 `artifact-v1`，每个 channel 的 `schemaVersion` 为 `delivery-channel-v1`。
- `scripts/validate_repo.py` 验证 committed artifact、deterministic builder output 与多条 forced-failure case：workload_type_regression_reuse / kind_regression_reuse / schema_regression_reuse / envelope_schema_regression_reuse / channel_envelope_schema_regression_reuse / creator_owns_verdict / creator_must_not_own_verdict_disabled / task_spec_binding_regression_schema_reuse / task_spec_binding_hash_drift / capability_manifest_binding_regression_schema_reuse / capability_manifest_binding_hash_drift / verifier_result_binding_regression_schema_reuse / verifier_result_binding_hash_drift / run_event_log_binding_regression_schema_reuse / run_event_log_binding_hash_drift / policy_external_side_effects_allowed / policy_external_communication_allowed / policy_repository_mutation_allowed / channels_empty / channel_missing_required / channel_duplicate_name / channel_kind_unknown / channel_status_real_delivery / channel_status_inconsistent_with_gates / channel_unlocked_when_blocked / channel_policy_external_side_effects_allowed / channel_policy_external_communication_allowed / channel_approval_ref_path_invalid / channel_evidence_ref_mismatch / channel_payload_rendering_external / channel_schema_regression_reuse / required_verdict_not_passed / verdict_gate_unlocked_when_verdict_failed / run_terminal_state_gate_drift / approval_gate_granted_when_rejected / approval_gate_unlocked_when_not_granted / coordination_binding_missing_broadcast / coordination_binding_wrong_path / reuses_regression_email_draft / reuses_phase9_delivery_report / reuses_send_email_capability / non_regression_reuse_path_missing / channel_count_drift / any_channel_unlocked_drift / source_hash_mismatch_task_spec / source_hash_mismatch_capability_manifest / source_hash_mismatch_verifier_result / source_hash_mismatch_run_event_log / source_hash_mismatch_broadcast。
- `EvaluationReportV1` 把 DeliveryManifestV1 artifact 纳入 source hash，并把下一推荐切片推进为 workload-independent `PolicyV1 / PolicyManifestV1` ArtifactV1 subtype。

实现摘要：

- 新增 `scripts/delivery_manifest.py`，包含 schema 常量、5 个 synthetic deterministic channel、builder、validator、deterministic CLI（`--out` / `--validate`），以及 ArtifactV1 envelope / TaskSpec & CapabilityManifest & VerifierResult & RunEventLog quad binding / 五件套 coordination binding 逻辑；channelStatus 由 verdict / run terminal state / approval 三重门派生。
- 新增 committed `artifacts/delivery/post_mvp_test_execution_delivery_manifest.json`。
- 更新 `scripts/validate_repo.py`：REQUIRED_FILES 增加 `scripts/delivery_manifest.py` 与 `artifacts/delivery/post_mvp_test_execution_delivery_manifest.json`，PLAN markers 增加 `DeliveryManifestV1` / `delivery-manifest-v1` / `delivery-channel-v1` / `post_mvp_test_execution_delivery_manifest.json`，新增 committed artifact 校验、deterministic CLI replay 与 47 条 forced-failure case。
- 更新 `scripts/evaluation_report.py` 与 `artifacts/evaluation/phase9_mvp_evaluation_report.json`：把 DeliveryManifestV1 加入 source hash，并把 `nextRecommendedSlice` 推进为 workload-independent `PolicyV1 / PolicyManifestV1` ArtifactV1 subtype。

Build vs Integrate：

- Build：`DeliveryManifestV1` schema、deterministic builder / validator / CLI、committed artifact、47 条 forced-failure case、`EvaluationReportV1` source hash 集成、Verifier ↔ RunEventLog ↔ Approval ↔ Delivery 一致性约束。
- Integrate later：真实交付通道（IDE review surface、ticket system、PR review feed、internal dashboard）、跨 workload `DeliveryManifestV1` catalogue、ArtifactV1 envelope 抽取为独立 schema 文件、`PolicyV1` / `RunV1` / `StepV1` 等下一批 OS kernel primitive。
...
- Baseline /usr/bin/python3 scripts/validate_repo.py before edits：通过。
- /usr/bin/python3 -m py_compile scripts/*.py：通过。
- /usr/bin/python3 scripts/delivery_manifest.py --out artifacts/delivery/post_mvp_test_execution_delivery_manifest.json：通过。
- /usr/bin/python3 scripts/delivery_manifest.py --validate artifacts/delivery/post_mvp_test_execution_delivery_manifest.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --report-out artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --validate-report artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/validate_repo.py：通过，覆盖 DeliveryManifestV1 committed artifact、deterministic builder output 与 47 条 forced-failure case。
- git diff --check：通过。
```

剩余风险：

- `DeliveryManifestV1` 仍是 static contract artifact，不驱动任何真实交付通道；下一切片应给出 workload-independent `PolicyV1 / PolicyManifestV1` ArtifactV1 subtype，把 OS kernel 的策略原语在第二 workload 上独立抽象出来。
- 当前 artifact 复用 first workload 的 5 件套 coordination contract artifact 与 phase9 human approval decision fixture 作为 source fixture；`Code Patch / Review Loop` 与 `PR Review` 的 TaskSpec / Artifact / DeliveryManifest subtype 仍未实现。
- `ArtifactV1` envelope 目前仍通过 `artifactEnvelopeSchemaVersion` 字段表达；未来需要把 `regression-result-artifact-v1` / `test-execution-result-v1` / `failure-triage-report-v1` / `verifier-result-v1` / `capability-manifest-v1` / `run-event-log-v1` / `delivery-manifest-v1` 抽出共同 base schema（真正的 `artifact-v1` envelope）。

下一轮建议：

```text
新增 workload-independent PolicyV1 / PolicyManifestV1 ArtifactV1 subtype contract artifact：
...
### 2026-05-14 11:00 UTC Automation Sprint：EvidenceListV1 Workload-Independent Evidence Primitive Gate

Active phase：post-MVP Kernel Generalization / Multi-Workload Expansion（Phase 1a-9 contract-only MVP 已满足，Agent Coordination Layer 五件套 contract 完成，第二 workload 的 TaskSpec envelope、TestExecutionResultV1、FailureTriageReportV1、VerifierResultV1、CapabilityManifestV1、RunEventLogV1、DeliveryManifestV1、PolicyManifestV1 均已 land，正在闭合 OS kernel 的 Evidence 原语，把「证据」从 first-workload regression `evidence.json` (`evidence-list-v1`) 抽象为 workload-independent ArtifactV1 子类型）。

Selected slice：

```text
Add EvidenceListV1 ArtifactV1 subtype contract artifact at artifacts/evidence/post_mvp_test_execution_evidence_list.json so /usr/bin/python3 scripts/validate_repo.py verifies a workload-independent Evidence primitive for Test Execution / Failure Triage, declaring evidence-list-artifact-v1 / evidence-item-v1 / evidence-source-v1 schemas distinct from the first-workload regression evidence-list-v1, partitioning the evidenceKindDomain into a workload-independent set (test_case_outcome_record / stack_trace_record / log_excerpt / diff_snippet / build_log_excerpt / screenshot_redaction_record / triage_finding_excerpt / evidence_metadata_record), enforcing the six permission flags to false, binding TestExecutionTaskSpecV1 / CapabilityManifestV1 / RunEventLogV1 / TestExecutionResultV1 / FailureTriageReportV1 / VerifierResultV1 / DeliveryManifestV1 / PolicyManifestV1 / HumanApprovalDecisionV1 by content hash plus the five Agent Coordination Layer contracts, mirroring TestExecutionResultV1.evidenceList by id / kind / contentHash, requiring every FailureTriageReportV1 supportingEvidenceId to resolve into the envelope, declaring verifier-runtime-v1 as the verdict owner with creatorMustNotOwnVerdict=true, and explicitly forbidding regression_result / email_draft / send_email / regression-task-spec-v1 / regression-result-artifact-v1 / reusesRegressionEvidenceList=false / reusesSendEmailCapability=false.
```

为什么这是下一步：

- 上一轮 `EvaluationReportV1.nextRecommendedSlice` 明确要求新增 workload-independent `EvidenceV1 / EvidenceListV1` ArtifactV1 subtype，把 OS kernel 的证据原语在第二 workload 上独立抽象出来。
- Post-MVP slice priority #1（generalize workload-independent kernel primitive：`EvidenceV1`）与 #2（add second workload：`Test Execution / Failure Triage`）继续叠加：在 `ArtifactV1` envelope 上引入第九个具体 subtype `evidence-list-artifact-v1`，与 `regression-result-artifact-v1` / `test-execution-result-v1` / `failure-triage-report-v1` / `verifier-result-v1` / `capability-manifest-v1` / `run-event-log-v1` / `delivery-manifest-v1` / `policy-manifest-v1` 并列。
- 这是 OS kernel 的 `Evidence` 抽象第一次以 workload-independent 形式独立成 ArtifactV1 子类型。既有的 `artifacts/runs/*/evidence.json`（schemaVersion `evidence-list-v1`）是 first-workload regression demo 的内嵌 evidence；既有的 `TestExecutionResultV1.evidenceList` 仅是 result artifact 内的子数组。新 artifact 在显式 `regressionWorkloadIsolation.reusesRegressionEvidenceList=false` 的前提下，用新的 `evidence-list-artifact-v1` envelope、`evidence-item-v1` / `evidence-source-v1` 子 schema，把每一条证据抽出来成为独立可绑定 / 可审计 / 可校验的 workload-independent 对象，并与 `TestExecutionResultV1.evidenceList` 通过 id / kind / contentHash 一一对齐。
- `.github/workflows/validate.yml` 与 `.github/workflows/auto-merge-cursor-pr.yml` 均存在；本轮 baseline `/usr/bin/python3 scripts/validate_repo.py` 已通过。

OS kernel primitive advanced：

- 在 `ArtifactV1` envelope 上引入第九个 subtype `evidence-list-artifact-v1`；`artifactEnvelopeSchemaVersion=artifact-v1` 显式声明 envelope 复用。未来 `CodePatchEvidenceListV1`、`PRReviewEvidenceListV1`、`DocumentationUpdateEvidenceListV1`、`IncidentAnalysisEvidenceListV1` 等可以复用同一 envelope。
- `EvidenceV1` / `evidence-item-v1` / `evidence-source-v1`：第一次把 OS kernel 中的 `Evidence` 抽象作为独立 schema 落地；evidenceKindDomain、evidenceSourceKindDomain、evidence → capability 绑定、redaction policy 绑定全部 workload-independent。
- `TestExecutionResult ↔ EvidenceList` 一致性：每个 `EvidenceItemV1.evidenceId` 必须与 `TestExecutionResultV1.evidenceList[*].id` 一一对应，`evidenceKind` 与 `contentHash` 必须严格匹配；validator 显式拒绝增删、id drift、hash drift。
- `FailureTriageReport ↔ EvidenceList` 一致性：`failureTriageEvidenceCoverage` 字段把 `FailureTriageReportV1.triageFindings[*].supportingEvidenceIds` 全集合解析到 evidence envelope，`evidenceListCoversTriage=true` 是 invariant；validator 显式拒绝 unresolved triage evidence。
- `Verifier ↔ RunEvent ↔ Policy ↔ Evidence` 三重门：`promotionConditions` 把 `VerifierResultV1.verdict==passed` / `RunEventLogV1.terminalState==completed` / `PolicyManifestV1.unlockConditions.unlocked==true` 钉为 evidence promotion 条件。任何一个 gate 失败 → `promoted=false`。

Non-regression workload reuse path：

- `Test Execution / Failure Triage`：本 artifact 直接覆盖；4 条 evidence item + 1 条 redaction policy binding + 完整 9 件套 binding。
- `Code Patch / Review Loop`：未来 `CodePatchEvidenceListV1` 可复用同一 `evidence-list-artifact-v1` envelope、`evidence-item-v1` / `evidence-source-v1` 子 schema、evidenceKind → capability 一致性约束；evidenceKind 切换到 `diff_snippet` / `build_log_excerpt` 等仍然 workload-independent 的名字。
- `PR Review` / `Documentation Update` / `Incident / Log Analysis`：同理。本 artifact 的 `nonRegressionReusePath` 字段已声明 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review` 三条路径。

Coordination protocol / invariant clarified：

- `coordinationBinding` 字段强制 EvidenceListV1 绑定 Agent Coordination Layer 五件套 contract artifact，并通过 `sourceArtifacts[].contentHash` 把上游 coordination / task spec / capability manifest / run event log / test execution result / failure triage report / verifier result / delivery manifest / policy manifest / approval decision contract 当作不可变前提；任何一个上游 contract 漂移都会让本 artifact validate 失败。
- `verifierVerdictOwner=verifier-runtime-v1` 与 `verifierExpectations.creatorMustNotOwnVerdict=true` 把 CreatorVerifierPairingV1 不变量延伸到证据层：creator 写入 evidence draft，但 verifier 拥有 acceptance；envelope 的 `promotionConditions.promoted=true` 不能由 creator 单方面声明。
- `permissionFlagDomain` / `permissionFlags` 对 6 个 workload-independent 权限位全部强制为 `false`，与既有 BroadcastSubscriptionManifestV1 / NegotiationRecordV1 / DelegationManifestV1 / TestExecutionTaskSpecV1 / TestExecutionResultV1 / FailureTriageReportV1 / VerifierResultV1 / CapabilityManifestV1 / RunEventLogV1 / DeliveryManifestV1 / PolicyManifestV1 的 policy invariant 对齐：证据原语自身不允许触发任何 side effect / external communication / public access。

为什么这不是另一个 regression/email fixture：

- artifact 路径为 `artifacts/evidence/post_mvp_test_execution_evidence_list.json`（独立于 `artifacts/runs/*/evidence.json` 这些 first-workload regression evidence artifact），scope 为 `test_execution_failure_triage_evidence_list_contract`，不是 regression evidence、email draft、approval lifecycle、delivery unlock 或 policy unlock。
- `workloadType` 必须等于 `test_execution_failure_triage`，validator 显式拒绝把它改成 `read_only_regression_evidence_demo` 或任何 regression workload；`kind` 必须为 `EvidenceListV1`，`schemaVersion` 必须为 `evidence-list-artifact-v1`（区别于 first-workload `evidence-list-v1`），每条 item 的 `schemaVersion` 必须为 `evidence-item-v1`，每个 source 的 `schemaVersion` 必须为 `evidence-source-v1`。
- `regressionWorkloadIsolation.forbiddenFields` 包含 `regression_result` / `regression_result.json` / `email_draft` / `email_draft.md` / `send_email` / `regression-task-spec-v1` / `regression-result-artifact-v1`；新增 `reusesRegressionEvidenceList=false` / `reusesRegressionResultArtifact=false` / `reusesSendEmailCapability=false` 把本 artifact 与 first-workload regression evidence artifact 显式隔离。defensive scan 拒绝这些 token 在 forbiddenFields 之外再次出现。
- `nonRegressionReusePath` 必须包含 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review`，强制本 artifact 至少有两个 non-regression 复用路径。

Acceptance criteria：

- `scripts/evidence_artifact.py` 提供 `build_evidence_list_artifact` / `validate_evidence_list_artifact` 与 CLI（`--out` / `--validate`）。
- 新增 committed `artifacts/evidence/post_mvp_test_execution_evidence_list.json`，schemaVersion 为 `evidence-list-artifact-v1`，artifactEnvelopeSchemaVersion 为 `artifact-v1`，每条 item 的 `schemaVersion` 为 `evidence-item-v1`，每个 source 的 `schemaVersion` 为 `evidence-source-v1`。
- `scripts/validate_repo.py` 验证 committed artifact、deterministic builder output 与约 50 条 forced-failure case（workload_type / kind / schema / envelope / verdict ownership / permission flags / domain drift / evidence item schema / kind / capability / source / hash / id / count / 9 binding pair / promotion gates / failure triage coverage / coordination binding / regression isolation / non-regression reuse path / source hash mismatches）。
- `EvaluationReportV1` 把 EvidenceListV1 artifact 纳入 source hash，并把下一推荐切片推进为 workload-independent `ObservationV1` ArtifactV1 subtype。

实现摘要：

- 新增 `scripts/evidence_artifact.py`，包含 schema 常量、4 条 synthetic deterministic evidence item（mirror TestExecutionResultV1.evidenceList）、builder、validator、deterministic CLI（`--out` / `--validate`），以及 ArtifactV1 envelope / 9 向 binding / 五件套 coordination binding 逻辑；`failureTriageEvidenceCoverage` 自动计算 triage cited ↔ envelope 交集；`promotionConditions` 由 verdict / run terminal state / policy unlock 三重门派生。
- 新增 committed `artifacts/evidence/post_mvp_test_execution_evidence_list.json`。
- 更新 `scripts/validate_repo.py`：REQUIRED_FILES 增加 `scripts/evidence_artifact.py` 与 `artifacts/evidence/post_mvp_test_execution_evidence_list.json`，PLAN markers 增加 `evidence-list-artifact-v1` / `evidence-item-v1` / `evidence-source-v1` / `post_mvp_test_execution_evidence_list.json`，新增 committed artifact 校验、deterministic CLI replay 与约 50 条 forced-failure case。
- 更新 `scripts/evaluation_report.py` 与 `artifacts/evaluation/phase9_mvp_evaluation_report.json`：把 EvidenceListV1 加入 source hash，并把 `nextRecommendedSlice` 推进为 workload-independent `ObservationV1` ArtifactV1 subtype。

Build vs Integrate：

- Build：`EvidenceListV1` schema、deterministic builder / validator / CLI、committed artifact、50 条 forced-failure case、`EvaluationReportV1` source hash 集成、TestExecutionResult ↔ EvidenceList 一致性约束、FailureTriageReport ↔ EvidenceList 覆盖率约束、Verifier ↔ RunEventLog ↔ Policy 三重 promotion 门、9 向 binding。
- Integrate later：真实证据存储（Evidence Graph 数据库 / artifact registry）、跨 workload `EvidenceListV1` catalogue、`ArtifactV1` envelope 抽取为独立 schema 文件、`ObservationV1` / `StepV1` / `RunV1` 等下一批 OS kernel primitive。
...
- Baseline /usr/bin/python3 scripts/validate_repo.py before edits：通过。
- /usr/bin/python3 -m py_compile scripts/*.py：通过。
- /usr/bin/python3 scripts/evidence_artifact.py --out artifacts/evidence/post_mvp_test_execution_evidence_list.json：通过。
- /usr/bin/python3 scripts/evidence_artifact.py --validate artifacts/evidence/post_mvp_test_execution_evidence_list.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --report-out artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --validate-report artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/validate_repo.py：通过，覆盖 EvidenceListV1 committed artifact、deterministic builder output 与约 50 条 forced-failure case。
- git diff --check：通过。
```

剩余风险：

- `EvidenceListV1` 仍是 static contract artifact，不驱动任何真实证据存储 / Evidence Graph runtime；下一切片应给出 workload-independent `ObservationV1` ArtifactV1 subtype，把 OS kernel 的观察原语在第二 workload 上独立抽象出来。
- 当前 artifact 复用 first workload 的 5 件套 coordination contract artifact 与 phase9 human approval decision fixture 作为 source fixture；`Code Patch / Review Loop` 与 `PR Review` 的 TaskSpec / Artifact / EvidenceList subtype 仍未实现。
- `ArtifactV1` envelope 目前仍通过 `artifactEnvelopeSchemaVersion` 字段表达；未来需要把所有 9 个 subtype（`regression-result-artifact-v1` / `test-execution-result-v1` / `failure-triage-report-v1` / `verifier-result-v1` / `capability-manifest-v1` / `run-event-log-v1` / `delivery-manifest-v1` / `policy-manifest-v1` / `evidence-list-artifact-v1`）抽出共同 base schema（真正的 `artifact-v1` envelope）。

下一轮建议：

```text
新增 workload-independent ObservationV1 ArtifactV1 subtype contract artifact：
...
### 2026-05-14 11:40 UTC Automation Sprint：RunV1 Workload-Independent Run Primitive Gate

Active phase：post-MVP Kernel Generalization / Multi-Workload Expansion（Phase 1a-9 contract-only MVP 已满足，Agent Coordination Layer 五件套 contract 完成，第二 workload 的 TaskSpec envelope、TestExecutionResultV1、FailureTriageReportV1、VerifierResultV1、CapabilityManifestV1、RunEventLogV1、DeliveryManifestV1、PolicyManifestV1、EvidenceListV1、ObservationListV1 均已 land，正在闭合 OS kernel 的 Run 原语，把「一次工程任务的完整 Run」从分散在 Run、Step、Event、Observation、Evidence、Verifier、Delivery、Policy、Approval 各 artifact 中的隐式状态抽象为一个 workload-independent ArtifactV1 子类型）。

Selected slice：

```text
Add RunV1 ArtifactV1 subtype contract artifact at artifacts/runs/post_mvp_test_execution_run.json so /usr/bin/python3 scripts/validate_repo.py verifies a workload-independent Run primitive for Test Execution / Failure Triage, declaring run-artifact-v1 schema distinct from the first-workload regression artifacts/runs/<fixture>/run.json artifact, declaring workload-independent runId / runState / startedAt / terminalAt / eventCount / observationCount / evidenceCount / runControlStateMachine fields anchored on the kernel-level run-state-machine-v1 state machine, enforcing the six permission flags to false, binding TestExecutionTaskSpecV1 / CapabilityManifestV1 / RunEventLogV1 / ObservationListV1 / EvidenceListV1 / VerifierResultV1 / DeliveryManifestV1 / PolicyManifestV1 / HumanApprovalDecisionV1 by content hash plus the five Agent Coordination Layer contracts, requiring runState=RunEventLogV1.terminalState, startedAt/terminalAt to equal the first/last RunEventLogV1.events timestamp, eventCount/observationCount/evidenceCount to equal the upstream counts, every observed (previous,current) runControlStateAfter pair to lie inside allowedRunControlTransitions, declaring verifier-runtime-v1 as the verdict owner with creatorMustNotOwnVerdict=true, and explicitly forbidding regression_result / email_draft / send_email / regression-task-spec-v1 / regression-result-artifact-v1 / reusesRegressionRunArtifact=false / reusesRegressionEventsArtifact=false / reusesSendEmailCapability=false.
```

为什么这是下一步：

- 上一轮 `EvaluationReportV1.nextRecommendedSlice` 明确要求新增 workload-independent `RunV1` ArtifactV1 subtype，把 OS kernel 的 Run 原语在第二 workload 上独立抽象出来。
- Post-MVP slice priority #1（generalize workload-independent kernel primitive：`RunV1`）与 #2（add second workload：`Test Execution / Failure Triage`）继续叠加：在 `ArtifactV1` envelope 上引入第十一个具体 subtype `run-artifact-v1`，与既有十个 subtype 并列，且第一次让 `Run` 抽象在新的 envelope 中把 TaskSpec / Capability / RunEventLog / Observation / Evidence / Verifier / Delivery / Policy / Approval 九件套全部用 content hash 钉死在一处。
- `.github/workflows/validate.yml` 与 `.github/workflows/auto-merge-cursor-pr.yml` 均存在；本轮 baseline `/usr/bin/python3 scripts/validate_repo.py` 已通过。

OS kernel primitive advanced：

- 在 `ArtifactV1` envelope 上引入第十一个 subtype `run-artifact-v1`；`artifactEnvelopeSchemaVersion=artifact-v1` 显式声明 envelope 复用。未来 `CodePatchRunV1` / `PRReviewRunV1` / `DocumentationUpdateRunV1` / `IncidentAnalysisRunV1` 等可以复用同一 envelope。
- `RunV1`：第一次把 OS kernel 中的 `Run` 抽象作为独立 schema 落地。`runControlStateMachine` 子 schema 在 artifact 层面把 `run-state-machine-v1` 状态机（`runControlStates` / `initialRunControlState` / `terminalRunControlStates` / `allowedRunControlTransitions`）和当前 run 实际观察到的 `observedRunControlTransitions` 同时显式化，validator 强制每个 (previous, current) 对都在 allow set 内，跨 workload 复用。
- `RunV1 ↔ RunEventLog` 一致性：`runState` 必须等于 `RunEventLogV1.terminalState`；`startedAt` / `terminalAt` 必须等于 `events[0]/[-1].timestampMs`；`eventCount` 必须等于 `RunEventLogV1.eventCount`；validator 显式拒绝任意漂移。
- `RunV1 ↔ ObservationList ↔ EvidenceList` 一致性：`observationCount` 必须等于 `ObservationListV1.observationItemCount` 且必须等于 `eventCount`；`evidenceCount` 必须等于 `EvidenceListV1.evidenceItemCount`，OS kernel 第一次拥有机器可验证的 (Run → RunEventLog → Observation → Evidence) 计数同构约束。

Non-regression workload reuse path：

- `Test Execution / Failure Triage`：本 artifact 直接覆盖；12 events / 12 observations / 4 evidences 全部使用 workload-independent 字段。
- `Code Patch / Review Loop`：未来 `CodePatchRunV1` 可复用同一 `run-artifact-v1` envelope、`run-state-machine-v1` 状态机、9 件套 binding 与 (Run → RunEventLog → Observation → Evidence) 计数同构约束；只需要替换上游 9 件套 contract artifact。
- `PR Review` / `Documentation Update` / `Incident / Log Analysis`：同理。本 artifact 的 `nonRegressionReusePath` 字段已声明 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review` 三条路径。

Coordination protocol / invariant clarified：

- `coordinationBinding` 字段强制 RunV1 绑定 Agent Coordination Layer 五件套 contract artifact，并通过 `sourceArtifacts[].contentHash` 把上游 9 件套 contract 当作不可变前提；任何一个上游 contract 漂移都会让本 artifact validate 失败。
- `verifierVerdictOwner=verifier-runtime-v1` 与 `verifierExpectations.creatorMustNotOwnVerdict=true` 把 CreatorVerifierPairingV1 不变量延伸到 Run 层：creator 写入 run draft，但 verifier 拥有 acceptance；envelope 的 `promotionConditions.promoted=true` 不能由 creator 单方面声明。
- `permissionFlagDomain` / `permissionFlags` 对 6 个 workload-independent 权限位全部强制为 `false`，与既有 BroadcastSubscriptionManifestV1 / NegotiationRecordV1 / DelegationManifestV1 / TestExecutionTaskSpecV1 / TestExecutionResultV1 / FailureTriageReportV1 / VerifierResultV1 / CapabilityManifestV1 / RunEventLogV1 / DeliveryManifestV1 / PolicyManifestV1 / EvidenceListV1 / ObservationListV1 的 policy invariant 对齐：Run 原语自身不允许触发任何 side effect / external communication / public access。

为什么这不是另一个 regression/email fixture：

- artifact 路径为 `artifacts/runs/post_mvp_test_execution_run.json`（独立于 `artifacts/runs/<fixture>/run.json` 这些 first-workload regression run artifact 目录），scope 为 `test_execution_failure_triage_run_contract`，不是 regression run、email draft、approval lifecycle、delivery unlock 或 policy unlock。
- `workloadType` 必须等于 `test_execution_failure_triage`，validator 显式拒绝把它改成 `read_only_regression_evidence_demo` 或任何 regression workload；`kind` 必须为 `RunV1`，`schemaVersion` 必须为 `run-artifact-v1`（区别于任何 first-workload schema）。`runId` 必须为 `run-post-mvp-test-execution-failure-triage`，validator 显式拒绝 `run-regression-*` 复用。
- `regressionWorkloadIsolation.forbiddenFields` 包含 `regression_result` / `regression_result.json` / `email_draft` / `email_draft.md` / `send_email` / `regression-task-spec-v1` / `regression-result-artifact-v1`；新增 `reusesRegressionRunArtifact=false` / `reusesRegressionEventsArtifact=false` / `reusesRegressionResultArtifact=false` / `reusesSendEmailCapability=false` 把本 artifact 与 first-workload regression run/events/result artifact 显式隔离。defensive scan 拒绝这些 token 在 forbiddenFields 之外再次出现。
- `nonRegressionReusePath` 必须包含 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review`，强制本 artifact 至少有两个 non-regression 复用路径。

Acceptance criteria：

- `scripts/run_artifact.py` 提供 `build_run_artifact` / `validate_run_artifact` 与 CLI（`--out` / `--validate`）。
- 新增 committed `artifacts/runs/post_mvp_test_execution_run.json`，schemaVersion 为 `run-artifact-v1`，artifactEnvelopeSchemaVersion 为 `artifact-v1`，`runControlStateMachine.runControlStateMachineSchemaVersion` 为 `run-state-machine-v1`。
- `scripts/validate_repo.py` 验证 committed artifact、deterministic builder output 与约 45 条 forced-failure case（workload_type / kind / schema / envelope / verdict ownership / permission flags / runId / runState / startedAt / terminalAt / eventCount / observationCount / evidenceCount / runControlStateMachine schema / allowed transitions / observed transitions / initial state / 9 binding pair regression-schema reuse / hash drift / approvalGranted drift / promotion gates / coordination binding / regression isolation / non-regression reuse path / 10 source hash mismatches）。
- `EvaluationReportV1` 把 RunV1 artifact 纳入 source hash，并把下一推荐切片推进为 workload-independent `StepListV1 / StepV1` ArtifactV1 subtype。

实现摘要：

- 新增 `scripts/run_artifact.py`，包含 schema 常量、builder、validator、deterministic CLI（`--out` / `--validate`），以及 ArtifactV1 envelope / 9 向 binding / 五件套 coordination binding 逻辑；`runControlStateMachine.observedRunControlTransitions` 自动从 RunEventLogV1.events 推导；`promotionConditions` 由 verdict / run terminal state / policy unlock 三重门派生。
- 新增 committed `artifacts/runs/post_mvp_test_execution_run.json`。
- 更新 `scripts/validate_repo.py`：REQUIRED_FILES 增加 `scripts/run_artifact.py` 与 `artifacts/runs/post_mvp_test_execution_run.json`，PLAN markers 增加 `RunV1` / `run-artifact-v1` / `post_mvp_test_execution_run.json`，新增 committed artifact 校验、deterministic CLI replay 与约 45 条 forced-failure case。
- 更新 `scripts/evaluation_report.py` 与 `artifacts/evaluation/phase9_mvp_evaluation_report.json`：把 RunV1 加入 source hash，并把 `nextRecommendedSlice` 推进为 workload-independent `StepListV1 / StepV1` ArtifactV1 subtype。

Build vs Integrate：

- Build：`RunV1` schema、deterministic builder / validator / CLI、committed artifact、约 45 条 forced-failure case、`EvaluationReportV1` source hash 集成、(Run → RunEventLog → Observation → Evidence) 计数同构约束、Verifier ↔ RunEventLog ↔ Policy 三重 promotion 门、9 向 binding。
- Integrate later：真实 Run 状态机 runtime / event sourcing / durable run store binding；跨 workload `RunV1` catalogue；`ArtifactV1` envelope 抽取为独立 schema 文件；`StepV1` / `RunControlV1` runtime / 真实 scheduler 等下一批 OS kernel primitive。
...
- Baseline /usr/bin/python3 scripts/validate_repo.py before edits：通过。
- /usr/bin/python3 -m py_compile scripts/*.py：通过。
- /usr/bin/python3 scripts/run_artifact.py --out artifacts/runs/post_mvp_test_execution_run.json：通过。
- /usr/bin/python3 scripts/run_artifact.py --validate artifacts/runs/post_mvp_test_execution_run.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --report-out artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --validate-report artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/validate_repo.py：通过，覆盖 RunV1 committed artifact、deterministic builder output 与约 45 条 forced-failure case。
- git diff --check：通过。
```

剩余风险：

- `RunV1` 仍是 static contract artifact，不驱动任何真实 Run 状态机 runtime / scheduler / event sourcing；下一切片应给出 workload-independent `StepListV1 / StepV1` ArtifactV1 subtype，把 OS kernel 的 Step 原语在第二 workload 上独立抽象出来，并把 (Run → Step → RunEvent → Observation → Evidence) 五层因果链显式化。
- 当前 artifact 复用 first workload 的 5 件套 coordination contract artifact 与 phase9 human approval decision fixture 作为 source fixture；`Code Patch / Review Loop` 与 `PR Review` 的 TaskSpec / Artifact / RunV1 subtype 仍未实现。
- `ArtifactV1` envelope 目前仍通过 `artifactEnvelopeSchemaVersion` 字段表达；未来需要把所有 11 个 subtype（`regression-result-artifact-v1` / `test-execution-result-v1` / `failure-triage-report-v1` / `verifier-result-v1` / `capability-manifest-v1` / `run-event-log-v1` / `delivery-manifest-v1` / `policy-manifest-v1` / `evidence-list-artifact-v1` / `observation-list-artifact-v1` / `run-artifact-v1`）抽出共同 base schema（真正的 `artifact-v1` envelope）。

下一轮建议：

```text
新增 workload-independent StepListV1 / StepV1 ArtifactV1 subtype contract artifact：
...
### 2026-05-14 13:00 UTC Automation Sprint：CapabilityV1 Workload-Independent Capability Primitive Gate

Active phase：post-MVP Kernel Generalization / Multi-Workload Expansion（Phase 1a-9 contract-only MVP 已满足，Agent Coordination Layer 五件套 contract 完成，第二 workload 的 TaskSpec envelope、TestExecutionResultV1、FailureTriageReportV1、VerifierResultV1、CapabilityManifestV1、RunEventLogV1、DeliveryManifestV1、PolicyManifestV1、EvidenceListV1、ObservationListV1、RunV1、StepListV1、ToolCallListV1、IntentV1 均已 land，正在闭合 OS kernel 的 Capability 原语：把「工具能力的注册条目」从 workload-specific `CapabilityManifestV1` 抽象为 workload-independent ArtifactV1 子类型 `CapabilityV1` / `capability-v1` / `capability-item-v1`，并把 ToolCallListV1.toolCallItems[*].capabilityRef → CapabilityV1.capabilityItems[*].capabilityId 的解析链路显式化）。

Selected slice：

```text
Add CapabilityV1 / capability-v1 ArtifactV1 subtype contract artifact at artifacts/capabilities/post_mvp_capability_v1_catalog.json so /usr/bin/python3 scripts/validate_repo.py verifies a workload-independent Capability primitive distinct from workload-specific CapabilityManifestV1: declare capability-v1 / capability-item-v1 schemas, partition the capabilityClassDomain into the workload-independent set (read_only_inspection / evidence_collection / artifact_writing / verifier_check / human_approval_request) and forbiddenCapabilityClasses (external_communication / repository_mutation / release_or_deploy / gui_or_browser_or_desktop_call / public_access), partition the sideEffectClassDomain into (read_only / evidence_writer / artifact_writer / verifier_check) and forbiddenSideEffectClasses (external / email_send_call / release_or_deploy / repository_mutation / gui_or_browser_or_desktop_call / public_access), enforce the six permission flags and per-capability permissionFlagOverrides to false, force every capability item to capabilityLifecycleState=registered, bind TestExecutionTaskSpecV1 / IntentV1 / RunV1 / StepListV1 / ToolCallListV1 / CapabilityManifestV1 / RunEventLogV1 / ObservationListV1 / EvidenceListV1 / VerifierResultV1 / DeliveryManifestV1 / PolicyManifestV1 / HumanApprovalDecisionV1 by content hash plus the five Agent Coordination Layer contracts, require ToolCallListV1.toolCallItems[*].capabilityRef to resolve into CapabilityV1.capabilityItems[*].capabilityId with everyToolCallRefResolved=true and everyCapabilityItemUsed=true, declare verifier-runtime-v1 as the verdict owner with creatorMustNotOwnVerdict=true, and explicitly forbid regression_result / email_draft / send_email / regression-task-spec-v1 / regression-result-artifact-v1 / reusesRegressionCapabilityArtifact=false / reusesSendEmailCapability=false.
```

为什么这是下一步：

- 上一轮 `EvaluationReportV1.nextRecommendedSlice` 明确要求新增 workload-independent `CapabilityV1` ArtifactV1 subtype，把 OS kernel 的 Capability 原语从工作负载特定的 `CapabilityManifestV1` 中抽出来，并显式表达 `ToolCallListV1.toolCallItems[*].capabilityRef → CapabilityV1.capabilityId` 的解析关系。
- Post-MVP slice priority #1（generalize workload-independent kernel primitive：`CapabilityV1`）与 #2（add second workload：`Test Execution / Failure Triage`）继续叠加：在 `ArtifactV1` envelope 上引入第十四个具体 subtype `capability-v1`，与既有十三个 subtype 并列。
- 这是 OS kernel 的 `Capability` 抽象第一次以 workload-independent 形式独立成 ArtifactV1 子类型。既有 `artifacts/capabilities/post_mvp_test_execution_capability_manifest.json` 把每个 capability 的运行参数（permission / sideEffect / timeoutMs / inputContract / outputContract / allowedEffectClasses / evidenceContract）封装成 workload-specific manifest；新 artifact 在显式 `regressionWorkloadIsolation.reusesRegressionCapabilityArtifact=false` 的前提下，用新的 `capability-v1` envelope、`capability-item-v1` 子 schema，把每一个 capability 注册项抽出来成为独立可绑定 / 可审计 / 可校验的 workload-independent 对象，并通过 `toolCallCoverage` 把 `ToolCallListV1.toolCallItems[*].capabilityRef` → `CapabilityV1.capabilityItems[*].capabilityId` 钉成不变量。
- `.github/workflows/validate.yml` 与 `.github/workflows/auto-merge-cursor-pr.yml` 均存在；本轮 baseline `/usr/bin/python3 scripts/validate_repo.py` 已通过。

OS kernel primitive advanced：

- 在 `ArtifactV1` envelope 上引入第十四个 subtype `capability-v1`；`artifactEnvelopeSchemaVersion=artifact-v1` 显式声明 envelope 复用。未来 `CodePatchCapabilityV1`、`PRReviewCapabilityV1`、`DocumentationUpdateCapabilityV1`、`IncidentAnalysisCapabilityV1` 等可以复用同一 envelope。
- `CapabilityV1` / `capability-v1`：第一次把 OS kernel 中的 `Capability` 抽象作为独立 schema 落地；capabilityClassDomain（`read_only_inspection` / `evidence_collection` / `artifact_writing` / `verifier_check` / `human_approval_request`）、sideEffectClassDomain（`read_only` / `evidence_writer` / `artifact_writer` / `verifier_check`）、forbiddenCapabilityClasses（`external_communication` / `repository_mutation` / `release_or_deploy` / `gui_or_browser_or_desktop_call` / `public_access`）、forbiddenSideEffectClasses（`external` / `email_send_call` / `release_or_deploy` / `repository_mutation` / `gui_or_browser_or_desktop_call` / `public_access`）、sourceAgentDomain（`creator_agent` / `verifier-runtime-v1`）、capabilityLifecycleStateDomain（`drafted` / `registered` / `deprecated` / `withdrawn`）全部 workload-independent。
- `capability-item-v1`：每个 capability 注册条目都拥有独立的 `capabilityId` / `capabilityClass` / `sideEffectClass` / `inputSchemaRef` / `outputSchemaRef` / `sourceAgentDomain` / `permissionFlagOverrides` / `verifierVerdictOwner` / `capabilityLifecycleState` / `description`，与 workload-specific `CapabilityManifestV1.capabilities[*]` 完全解耦。
- `ToolCallListV1 ↔ CapabilityV1` 一致性：`toolCallCoverage.everyToolCallRefResolved=true` 与 `everyCapabilityItemUsed=true` 钉为 invariant；`CapabilityV1.capabilityItems[*].capabilityId` 必须等于 `ToolCallListV1.toolCallItems[*].capabilityRef` 的唯一集合。OS kernel 第一次拥有「每一次 tool call 都能解析到一个 workload-independent capability 注册条目」的机器可验证证据。
- `Verifier ↔ RunEvent ↔ Policy ↔ Capability` 三重门：`promotionConditions` 把 `VerifierResultV1.verdict==passed` / `RunEventLogV1.terminalState==completed` / `PolicyManifestV1.unlockConditions.unlocked==true` 钉为 capability registry promotion 条件；任何一个 gate 失败 → `promoted=false`。

Non-regression workload reuse path：

- `Test Execution / Failure Triage`：本 artifact 直接覆盖；5 个 capability item（`read_test_execution_result` / `collect_test_failure_evidence` / `classify_test_failure` / `write_artifact` / `__run_kernel__`）覆盖 ToolCallListV1 全部 5 次 tool call 的 capabilityRef 全集。
- `Code Patch / Review Loop`：未来 `CodePatchCapabilityV1` 可复用同一 `capability-v1` envelope、`capability-item-v1` 子 schema、capabilityClassDomain、sideEffectClassDomain、sourceAgentDomain、capabilityLifecycleStateDomain；只需要替换 `capabilityItems` 内容并把 capabilityClass 落在同一 workload-independent 集合里。
- `PR Review` / `Documentation Update` / `Incident / Log Analysis`：同理。本 artifact 的 `nonRegressionReusePath` 字段已声明 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review` 三条路径。

Coordination protocol / invariant clarified：

- `coordinationBinding` 字段强制 CapabilityV1 绑定 Agent Coordination Layer 五件套 contract artifact，并通过 `sourceArtifacts[].contentHash` 把上游 11 件套 + Run + StepList + ToolCallList + Intent contract 当作不可变前提；任何一个上游 contract 漂移都会让本 artifact validate 失败。
- `verifierVerdictOwner=verifier-runtime-v1` 与 `verifierExpectations.creatorMustNotOwnVerdict=true` 把 CreatorVerifierPairingV1 不变量延伸到 Capability 层：creator 写入 capability draft，但 verifier 拥有 acceptance；envelope 的 `promotionConditions.promoted=true` 不能由 creator 单方面声明。
- `permissionFlagDomain` / `permissionFlags` / 每个 capabilityItem 的 `permissionFlagOverrides` 都对 6 个 workload-independent 权限位强制为 `false`；`forbiddenSideEffectClasses` 显式禁止 `external` / `email_send_call` / `release_or_deploy` / `repository_mutation` / `gui_or_browser_or_desktop_call` / `public_access` 等 6 类副作用，与既有 BroadcastSubscriptionManifestV1 / NegotiationRecordV1 / DelegationManifestV1 / TestExecutionTaskSpecV1 / TestExecutionResultV1 / FailureTriageReportV1 / VerifierResultV1 / CapabilityManifestV1 / RunEventLogV1 / DeliveryManifestV1 / PolicyManifestV1 / EvidenceListV1 / ObservationListV1 / RunV1 / StepListV1 / ToolCallListV1 / IntentV1 的 policy invariant 对齐：Capability 原语自身不允许触发任何 side effect / external communication / public access。

为什么这不是另一个 regression/email fixture：

- artifact 路径为 `artifacts/capabilities/post_mvp_capability_v1_catalog.json`（独立于既有 `artifacts/capabilities/post_mvp_test_execution_capability_manifest.json` 这个 workload-specific manifest 与 `artifacts/capabilities/phase3_capability_catalog.json` 这个 first-workload regression read-only capability 目录），scope 为 `workload_independent_capability_v1_kernel_contract`，不是 regression run、email draft、approval lifecycle、delivery unlock 或 policy unlock。
- `workloadType` 必须等于 `test_execution_failure_triage`，validator 显式拒绝把它改成 `read_only_regression_evidence_demo` 或任何 regression workload；`kind` 必须为 `CapabilityV1`，`schemaVersion` 必须为 `capability-v1`（区别于任何 first-workload `capability-envelope-v1` / `capability-metadata-v1` / `capability-catalog-v1` / `capability-manifest-v1` schema），每个 capabilityItem 的 `schemaVersion` 必须为 `capability-item-v1`。
- `regressionWorkloadIsolation.forbiddenFields` 包含 `regression_result` / `regression_result.json` / `email_draft` / `email_draft.md` / `send_email` / `regression-task-spec-v1` / `regression-result-artifact-v1`；新增 `reusesRegressionCapabilityArtifact=false` / `reusesRegressionTaskSpec=false` / `reusesRegressionResultArtifact=false` / `reusesSendEmailCapability=false` 把本 artifact 与 first-workload regression capability / task spec / result / send_email 显式隔离。defensive scan 拒绝这些 token 在 forbiddenFields 之外再次出现。
- `nonRegressionReusePath` 必须包含 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review`，强制本 artifact 至少有两个 non-regression 复用路径。

Acceptance criteria：

- `scripts/capability_kernel.py` 提供 `build_capability_kernel_artifact` / `validate_capability_kernel_artifact` 与 CLI（`--out` / `--validate`）。
- 新增 committed `artifacts/capabilities/post_mvp_capability_v1_catalog.json`，schemaVersion 为 `capability-v1`，artifactEnvelopeSchemaVersion 为 `artifact-v1`，capabilityItemSchemaVersion 为 `capability-item-v1`，capabilityCount=5。
- `scripts/validate_repo.py` 验证 committed artifact、deterministic builder output、deterministic CLI replay 与约 55 条 forced-failure case（workload_type / kind / schema / envelope / item schema / verdict ownership / permission flags / capability class domain / forbidden capability classes / side effect class domain / forbidden side effect classes / source agent domain / lifecycle state domain / required lifecycle state / per-item lifecycle state / per-item permission overrides / per-item source agent / per-item verifier owner / per-item input schema / per-item output schema / per-item capabilityId / per-item sequence / count drift / 13 binding pair / tool-call coverage everyResolved / tool-call coverage everyUsed / tool-call coverage capability ids drift / promotion gates / coordination binding / regression isolation / non-regression reuse path / 14 source hash mismatches）。
- `EvaluationReportV1` 把 CapabilityV1 artifact 纳入 source hash，并把下一推荐切片推进为 workload-independent `PolicyV1` ArtifactV1 subtype。

实现摘要：

- 新增 `scripts/capability_kernel.py`，包含 schema 常量、5 条 deterministic CAPABILITY_BLUEPRINT、builder、validator、deterministic CLI（`--out` / `--validate`），以及 ArtifactV1 envelope / 13 向 binding（含 IntentV1 + RunV1 + StepList + ToolCallList）/ 五件套 coordination binding 逻辑；`toolCallCoverage` 自动从 capabilityItems 与上游 ToolCallListV1 推导；`promotionConditions` 由 verdict / run terminal state / policy unlock 三重门派生。
- 新增 committed `artifacts/capabilities/post_mvp_capability_v1_catalog.json`，共 5 个 capability item：3 个 `read_only_inspection` / `evidence_collection` 类创建者能力 + 1 个 `artifact_writing` 类创建者能力 + 1 个 `verifier_check` 类验证者能力，全部 `capabilityLifecycleState=registered`。
- 更新 `scripts/validate_repo.py`：REQUIRED_FILES 增加 `scripts/capability_kernel.py` 与 `artifacts/capabilities/post_mvp_capability_v1_catalog.json`，PLAN markers 增加 `CapabilityV1` / `capability-v1` / `capability-item-v1` / `post_mvp_capability_v1_catalog.json`，新增 committed artifact 校验、deterministic CLI replay 与约 55 条 forced-failure case。
- 更新 `scripts/evaluation_report.py` 与 `artifacts/evaluation/phase9_mvp_evaluation_report.json`：把 CapabilityV1 加入 source hash，并把 `nextRecommendedSlice` 推进为 workload-independent `PolicyV1` ArtifactV1 subtype（policy-v1 / policy-item-v1，policyClass / policyScope / policyLifecycleState / unlockConditionRef）。

Build vs Integrate：

- Build：`CapabilityV1 / capability-item-v1` schema、deterministic builder / validator / CLI、committed artifact、约 55 条 forced-failure case、`EvaluationReportV1` source hash 集成、`ToolCallListV1.toolCallItems[*].capabilityRef → CapabilityV1.capabilityItems[*].capabilityId` 解析链路显式化、Verifier ↔ RunEventLog ↔ Policy 三重 promotion 门、13 向 binding、permissionFlagOverrides 不变量。
- Integrate later：真实 Capability 注册 runtime / capability registry / 真实 capability lifecycle 调度 / capability versioning / capability deprecation runtime；跨 workload `CapabilityV1` catalogue；`ArtifactV1` envelope 抽取为独立 schema 文件；`PolicyV1` / 真实 policy lifecycle runtime / 真实 capability ↔ policy enforcement runtime 等下一批 OS kernel primitive。
...
- Baseline /usr/bin/python3 scripts/validate_repo.py before edits：通过。
- /usr/bin/python3 -m py_compile scripts/*.py：通过。
- /usr/bin/python3 scripts/capability_kernel.py --out artifacts/capabilities/post_mvp_capability_v1_catalog.json：通过。
- /usr/bin/python3 scripts/capability_kernel.py --validate artifacts/capabilities/post_mvp_capability_v1_catalog.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --report-out artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --validate-report artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/validate_repo.py：通过，覆盖 CapabilityV1 committed artifact、deterministic CLI replay 与约 55 条 forced-failure case。
- git diff --check：通过。
```

剩余风险：

- `CapabilityV1` 仍是 static contract artifact，不驱动任何真实 capability registry / 真实 capability lifecycle 调度 / capability versioning / capability deprecation runtime；下一切片应给出 workload-independent `PolicyV1` ArtifactV1 subtype，把 OS kernel 的 Policy 原语在第二 workload 上独立抽象出来，并把 `PolicyManifestV1.unlockConditions` 显式 binding 到 `PolicyV1.policyItems[*].unlockConditionRef`。
- 当前 artifact 复用 first workload 的 5 件套 coordination contract artifact 与 phase9 human approval decision fixture 作为 source fixture；`Code Patch / Review Loop` 与 `PR Review` 的 CapabilityV1 subtype 仍未实现。
- `ArtifactV1` envelope 目前仍通过 `artifactEnvelopeSchemaVersion` 字段表达；未来需要把所有 15 个 subtype（`regression-result-artifact-v1` / `test-execution-result-v1` / `failure-triage-report-v1` / `verifier-result-v1` / `capability-manifest-v1` / `run-event-log-v1` / `delivery-manifest-v1` / `policy-manifest-v1` / `evidence-list-artifact-v1` / `observation-list-artifact-v1` / `run-artifact-v1` / `step-list-artifact-v1` / `tool-call-list-artifact-v1` / `intent-v1` / `capability-v1`）抽出共同 base schema（真正的 `artifact-v1` envelope）。

下一轮建议：

```text
新增 workload-independent PolicyV1 ArtifactV1 subtype contract artifact：
...
### 2026-05-14 13:20 UTC Automation Sprint：PolicyV1 Workload-Independent Policy Primitive Gate

Active phase：post-MVP Kernel Generalization / Multi-Workload Expansion（Phase 1a-9 contract-only MVP 已满足，Agent Coordination Layer 五件套 contract 完成，第二 workload 的 TaskSpec envelope、TestExecutionResultV1、FailureTriageReportV1、VerifierResultV1、CapabilityManifestV1、RunEventLogV1、DeliveryManifestV1、PolicyManifestV1、EvidenceListV1、ObservationListV1、RunV1、StepListV1、ToolCallListV1、IntentV1、CapabilityV1 均已 land，正在闭合 OS kernel 的 Policy 原语：把「策略条目」从 workload-specific `PolicyManifestV1` 抽象为 workload-independent ArtifactV1 子类型 `PolicyV1` / `policy-v1` / `policy-item-v1`，并把 `PolicyManifestV1.unlockConditions.<key>` → `PolicyV1.policyItems[*].unlockConditionRef` 的解析链路显式化）。

Selected slice：

```text
Add PolicyV1 / policy-v1 ArtifactV1 subtype contract artifact at artifacts/policy/post_mvp_policy_v1_catalog.json so /usr/bin/python3 scripts/validate_repo.py verifies a workload-independent Policy primitive distinct from workload-specific PolicyManifestV1: declare policy-v1 / policy-item-v1 schemas, partition the policyClassDomain into the workload-independent set (permission_policy / approval_policy / delivery_policy / external_side_effect_policy / regression_isolation_policy / coordination_policy) and forbiddenPolicyClasses (regression_result_policy / email_draft_policy / send_email_policy), partition the policyScopeDomain into (workload_independent_kernel / workload_specific / coordination_layer) and allowedCommittedPolicyScopes (workload_independent_kernel / coordination_layer), enforce the six permission flags and per-policyItem permissionFlagOverrides to false, force every policy item to policyLifecycleState=active and requiredFlag=false, bind TestExecutionTaskSpecV1 / IntentV1 / RunV1 / StepListV1 / ToolCallListV1 / CapabilityV1 / CapabilityManifestV1 / RunEventLogV1 / ObservationListV1 / EvidenceListV1 / VerifierResultV1 / DeliveryManifestV1 / PolicyManifestV1 / HumanApprovalDecisionV1 by content hash plus the five Agent Coordination Layer contracts, require every PolicyManifestV1.unlockConditions key to resolve into PolicyV1.policyItems[*].unlockConditionRef with everyUnlockConditionKeyCovered=true and everyPolicyItemReferencesValidKey=true, declare verifier-runtime-v1 as the verdict owner with creatorMustNotOwnVerdict=true, and explicitly forbid regression_result / email_draft / send_email / regression-task-spec-v1 / regression-result-artifact-v1 / reusesRegressionPolicyArtifact=false / reusesPhase7AdapterPolicyManifest=false / reusesPolicyUnlockDenialFixture=false / reusesSendEmailCapability=false.
```

为什么这是下一步：

- 上一轮 `EvaluationReportV1.nextRecommendedSlice` 明确要求新增 workload-independent `PolicyV1` ArtifactV1 subtype，把 OS kernel 的 Policy 原语从 workload-specific `PolicyManifestV1` 中抽出来，并显式表达 `PolicyManifestV1.unlockConditions.<key> → PolicyV1.policyItems[*].unlockConditionRef` 的解析关系。
- Post-MVP slice priority #1（generalize workload-independent kernel primitive：`PolicyV1`）与 #2（add second workload：`Test Execution / Failure Triage`）继续叠加：在 `ArtifactV1` envelope 上引入第十五个具体 subtype `policy-v1`，与既有十四个 subtype 并列。
- 这是 OS kernel 的 `Policy` 抽象第一次以 workload-independent 形式独立成 ArtifactV1 子类型。既有 `artifacts/policy/post_mvp_test_execution_policy_manifest.json` 把策略的运行细节（effectClasses / policyRules / revocationRules / unlockConditions）封装成 workload-specific manifest；新 artifact 在显式 `regressionWorkloadIsolation.reusesPhase7AdapterPolicyManifest=false` / `reusesPolicyUnlockDenialFixture=false` / `reusesRegressionPolicyArtifact=false` 的前提下，用新的 `policy-v1` envelope、`policy-item-v1` 子 schema，把每一个策略条目抽出来成为独立可绑定 / 可审计 / 可校验的 workload-independent 对象，并通过 `unlockConditionCoverage` 把 `PolicyManifestV1.unlockConditions.<key>` → `PolicyV1.policyItems[*].unlockConditionRef` 钉成不变量。
- `.github/workflows/validate.yml` 与 `.github/workflows/auto-merge-cursor-pr.yml` 均存在；本轮 baseline `/usr/bin/python3 scripts/validate_repo.py` 已通过。

OS kernel primitive advanced：

- 在 `ArtifactV1` envelope 上引入第十五个 subtype `policy-v1`；`artifactEnvelopeSchemaVersion=artifact-v1` 显式声明 envelope 复用。未来 `CodePatchPolicyV1`、`PRReviewPolicyV1`、`DocumentationUpdatePolicyV1`、`IncidentAnalysisPolicyV1` 等可以复用同一 envelope。
- `PolicyV1` / `policy-v1`：第一次把 OS kernel 中的 `Policy` 抽象作为独立 schema 落地；policyClassDomain（`permission_policy` / `approval_policy` / `delivery_policy` / `external_side_effect_policy` / `regression_isolation_policy` / `coordination_policy`）、policyScopeDomain（`workload_independent_kernel` / `workload_specific` / `coordination_layer`）、forbiddenPolicyClasses（`regression_result_policy` / `email_draft_policy` / `send_email_policy`）、policyLifecycleStateDomain（`drafted` / `active` / `deprecated` / `withdrawn`）全部 workload-independent。
- `policy-item-v1`：每个策略条目都拥有独立的 `policyId` / `policyClass` / `policyScope` / `policyLifecycleState` / `requiredFlag` / `verifierVerdictOwner` / `unlockConditionRef` / `permissionFlagOverrides` / `description`，与 workload-specific `PolicyManifestV1.policyRules[*]` 完全解耦。
- `PolicyManifestV1.unlockConditions ↔ PolicyV1.policyItems` 一致性：`unlockConditionCoverage.everyUnlockConditionKeyCovered=true` 与 `everyPolicyItemReferencesValidKey=true` 钉为 invariant；`PolicyV1.policyItems[*].unlockConditionRef` 的唯一集合必须等于 `PolicyManifestV1.unlockConditions` 的 key 集合。OS kernel 第一次拥有「每一个 policy unlock 条件都能解析到一个 workload-independent policy 条目」的机器可验证证据。
- `Verifier ↔ RunEventLog ↔ PolicyManifest` 三重门：`promotionConditions` 把 `VerifierResultV1.verdict==passed` / `RunEventLogV1.terminalState==completed` / `PolicyManifestV1.unlockConditions.unlocked==true` 钉为 policy promotion 条件；任何一个 gate 失败 → `promoted=false`。

Non-regression workload reuse path：

- `Test Execution / Failure Triage`：本 artifact 直接覆盖；11 个 policy item 一一映射到 `PolicyManifestV1.unlockConditions` 的 11 个 key（`approvalGranted` / `approvalRecordRef` / `approvalRecordSchemaVersion` / `currentApprovalDecision` / `currentTerminalRunState` / `currentVerdict` / `requiredApprovalDecision` / `requiredTerminalRunState` / `requiredVerdict` / `unlocked` / `verdictOwner`），并把 6 个 policy class 用满（permission_policy / approval_policy / delivery_policy / external_side_effect_policy / regression_isolation_policy / coordination_policy）。
- `Code Patch / Review Loop`：未来 `CodePatchPolicyV1` 可复用同一 `policy-v1` envelope、`policy-item-v1` 子 schema、policyClassDomain、policyScopeDomain、policyLifecycleStateDomain；只需要替换 `policyItems` 内容并把 policyClass 落在同一 workload-independent 集合里。
- `PR Review` / `Documentation Update` / `Incident / Log Analysis`：同理。本 artifact 的 `nonRegressionReusePath` 字段已声明 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review` 三条路径。

Coordination protocol / invariant clarified：

- `coordinationBinding` 字段强制 PolicyV1 绑定 Agent Coordination Layer 五件套 contract artifact，并通过 `sourceArtifacts[].contentHash` 把上游 12 件套 + Run + StepList + ToolCallList + Intent + CapabilityV1 contract 当作不可变前提；任何一个上游 contract 漂移都会让本 artifact validate 失败。
- `verifierVerdictOwner=verifier-runtime-v1` 与 `verifierExpectations.creatorMustNotOwnVerdict=true` 把 CreatorVerifierPairingV1 不变量延伸到 Policy 层：creator 写入 policy draft，但 verifier 拥有 acceptance；envelope 的 `promotionConditions.promoted=true` 不能由 creator 单方面声明。
- `permissionFlagDomain` / `permissionFlags` / 每个 policyItem 的 `permissionFlagOverrides` 都对 6 个 workload-independent 权限位强制为 `false`；`forbiddenPolicyClasses` 显式禁止 `regression_result_policy` / `email_draft_policy` / `send_email_policy`，与既有 BroadcastSubscriptionManifestV1 / NegotiationRecordV1 / DelegationManifestV1 / TestExecutionTaskSpecV1 / TestExecutionResultV1 / FailureTriageReportV1 / VerifierResultV1 / CapabilityManifestV1 / RunEventLogV1 / DeliveryManifestV1 / PolicyManifestV1 / EvidenceListV1 / ObservationListV1 / RunV1 / StepListV1 / ToolCallListV1 / IntentV1 / CapabilityV1 的 policy invariant 对齐：Policy 原语自身不允许携带任何 regression 或 email 子类，也不允许触发任何 side effect / external communication / public access。

为什么这不是另一个 regression/email fixture：

- artifact 路径为 `artifacts/policy/post_mvp_policy_v1_catalog.json`（独立于既有 `artifacts/policy/post_mvp_test_execution_policy_manifest.json` 这个 workload-specific manifest、`artifacts/computer_runtime/phase7_adapter_policy_manifest.json` 这个 adapter overlay 与 `artifacts/approval/post_mvp_policy_unlock_request_denied.json` 这个 denial fixture），scope 为 `workload_independent_policy_v1_kernel_contract`，不是 regression run、email draft、approval lifecycle、delivery unlock 或 policy unlock。
- `workloadType` 必须等于 `test_execution_failure_triage`，validator 显式拒绝把它改成 `read_only_regression_evidence_demo` 或任何 regression workload；`kind` 必须为 `PolicyV1`，`schemaVersion` 必须为 `policy-v1`（区别于任何 first-workload `policy-manifest-v1` / `adapter-policy-manifest-v1` / `policy-rule-v1` / `policy-revocation-rule-v1` schema），每个 policyItem 的 `schemaVersion` 必须为 `policy-item-v1`。
- `regressionWorkloadIsolation.forbiddenFields` 包含 `regression_result` / `regression_result.json` / `email_draft` / `email_draft.md` / `send_email` / `regression-task-spec-v1` / `regression-result-artifact-v1`；新增 `reusesRegressionPolicyArtifact=false` / `reusesRegressionTaskSpec=false` / `reusesRegressionResultArtifact=false` / `reusesPhase7AdapterPolicyManifest=false` / `reusesPolicyUnlockDenialFixture=false` / `reusesSendEmailCapability=false` 把本 artifact 与 first-workload regression policy / adapter policy / policy unlock denial / regression task spec / regression result / send_email 显式隔离。defensive scan 拒绝这些 token 在 forbiddenFields 之外再次出现。
- `nonRegressionReusePath` 必须包含 `test_execution_failure_triage` / `code_patch_review_loop` / `pr_review`，强制本 artifact 至少有两个 non-regression 复用路径。

Acceptance criteria：

- `scripts/policy_kernel.py` 提供 `build_policy_kernel_artifact` / `validate_policy_kernel_artifact` 与 CLI（`--out` / `--validate`）。
- 新增 committed `artifacts/policy/post_mvp_policy_v1_catalog.json`，schemaVersion 为 `policy-v1`，artifactEnvelopeSchemaVersion 为 `artifact-v1`，policyItemSchemaVersion 为 `policy-item-v1`，policyCount=11。
- `scripts/validate_repo.py` 验证 committed artifact、deterministic builder output、deterministic CLI replay 与约 55 条 forced-failure case（workload_type / kind / schema / envelope / item schema / verdict ownership / permission flags / policy class domain / forbidden policy classes / policy scope domain / allowed committed scopes / lifecycle state domain / required lifecycle state / required flag / per-item lifecycle state / per-item permission overrides / per-item verifier owner / per-item unlock condition ref / per-item policyId / per-item sequence / count drift / 14 binding pair / unlock-condition coverage everyCovered / everyValid / refs drift / capability-kernel binding schema / capability count / promotion gates / coordination binding / regression isolation / non-regression reuse path / 15 source hash mismatches）。
- `EvaluationReportV1` 把 PolicyV1 artifact 纳入 source hash，并把下一推荐切片推进为 workload-independent `RunEventV1` ArtifactV1 subtype。

实现摘要：

- 新增 `scripts/policy_kernel.py`，包含 schema 常量、11 条 deterministic POLICY_BLUEPRINT、builder、validator、deterministic CLI（`--out` / `--validate`），以及 ArtifactV1 envelope / 14 向 binding（含 CapabilityV1 + IntentV1 + RunV1 + StepList + ToolCallList）/ 五件套 coordination binding 逻辑；`unlockConditionCoverage` 自动从 policyItems 与上游 PolicyManifestV1 推导；`promotionConditions` 由 verdict / run terminal state / policy unlock 三重门派生。
- 新增 committed `artifacts/policy/post_mvp_policy_v1_catalog.json`，共 11 个 policy item：1 个 `permission_policy` / 5 个 `approval_policy` / 2 个 `delivery_policy` / 1 个 `external_side_effect_policy` / 1 个 `regression_isolation_policy` / 1 个 `coordination_policy`，全部 `policyLifecycleState=active` 且 `requiredFlag=false`。
- 更新 `scripts/validate_repo.py`：REQUIRED_FILES 增加 `scripts/policy_kernel.py` 与 `artifacts/policy/post_mvp_policy_v1_catalog.json`，PLAN markers 增加 `PolicyV1` / `policy-v1` / `policy-item-v1` / `post_mvp_policy_v1_catalog.json`，新增 committed artifact 校验、deterministic CLI replay 与约 55 条 forced-failure case。
- 更新 `scripts/evaluation_report.py` 与 `artifacts/evaluation/phase9_mvp_evaluation_report.json`：把 PolicyV1 加入 source hash，并把 `nextRecommendedSlice` 推进为 workload-independent `RunEventV1` ArtifactV1 subtype（run-event-v1 / run-event-item-v1，runEventClass / runEventScope / runEventLifecycleState / sourceEventRef）。

Build vs Integrate：

- Build：`PolicyV1 / policy-item-v1` schema、deterministic builder / validator / CLI、committed artifact、约 55 条 forced-failure case、`EvaluationReportV1` source hash 集成、`PolicyManifestV1.unlockConditions.<key> → PolicyV1.policyItems[*].unlockConditionRef` 解析链路显式化、Verifier ↔ RunEventLog ↔ PolicyManifest 三重 promotion 门、14 向 binding、permissionFlagOverrides / requiredFlag 不变量。
- Integrate later：真实 Policy enforcement runtime / policy registry / 真实 policy lifecycle 调度 / policy versioning / policy deprecation runtime；跨 workload `PolicyV1` catalogue；`ArtifactV1` envelope 抽取为独立 schema 文件；`RunEventV1` / 真实 run-event lifecycle runtime / 真实 capability ↔ policy enforcement runtime 等下一批 OS kernel primitive。
...
- Baseline /usr/bin/python3 scripts/validate_repo.py before edits：通过。
- /usr/bin/python3 -m py_compile scripts/*.py：通过。
- /usr/bin/python3 scripts/policy_kernel.py --out artifacts/policy/post_mvp_policy_v1_catalog.json：通过。
- /usr/bin/python3 scripts/policy_kernel.py --validate artifacts/policy/post_mvp_policy_v1_catalog.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --report-out artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/evaluation_report.py --validate-report artifacts/evaluation/phase9_mvp_evaluation_report.json：通过。
- /usr/bin/python3 scripts/validate_repo.py：通过，覆盖 PolicyV1 committed artifact、deterministic CLI replay 与约 55 条 forced-failure case。
- git diff --check：通过。
```

剩余风险：

- `PolicyV1` 仍是 static contract artifact，不驱动任何真实 policy enforcement runtime / 真实 policy lifecycle 调度 / policy versioning / policy deprecation runtime；下一切片应给出 workload-independent `RunEventV1` ArtifactV1 subtype，把 OS kernel 的 RunEvent 原语在第二 workload 上独立抽象出来，并把 `RunEventLogV1.events[*].type` 显式 binding 到 `RunEventV1.runEventItems[*].sourceEventRef`。
- 当前 artifact 复用 first workload 的 5 件套 coordination contract artifact 与 phase9 human approval decision fixture 作为 source fixture；`Code Patch / Review Loop` 与 `PR Review` 的 PolicyV1 subtype 仍未实现。
- `ArtifactV1` envelope 目前仍通过 `artifactEnvelopeSchemaVersion` 字段表达；未来需要把所有 16 个 subtype（`regression-result-artifact-v1` / `test-execution-result-v1` / `failure-triage-report-v1` / `verifier-result-v1` / `capability-manifest-v1` / `run-event-log-v1` / `delivery-manifest-v1` / `policy-manifest-v1` / `evidence-list-artifact-v1` / `observation-list-artifact-v1` / `run-artifact-v1` / `step-list-artifact-v1` / `tool-call-list-artifact-v1` / `intent-v1` / `capability-v1` / `policy-v1`）抽出共同 base schema（真正的 `artifact-v1` envelope）。
