...
6. Evidence List + Verifier Runtime：已固化 `LogEvidenceV1`、`RegressionResultArtifactV1`、email grounding 规则和 fixture gate；forced-failure validation 已证明 evidence refs 与 email grounding 被篡改时会产生 failed verifier report。
7. Local Read-only Runner：仅在 Phase 1a artifact packet、forced-failure verifier gate 和本 PR / GitHub checks 通过后，进入最小 read-only runner；仍不得引入 capability registry、SQLite event store、daemon、CUA、IDE 或 multi-agent。
8. CUA Adapter Contract：post-MVP，只定义 `computer.*` / `trajectory.*` schema，不实际集成。
9. Phase 1a Evidence Intake Review：已完成 happy-path artifact packet review 和 forced-failure verifier proof；后续只有真实脱敏日志差异、marker 误判、source locator 不足或 Phase 1b runner 输出问题才继续修改 Phase 1a schema/rules。
10. Evidence Packet Stop Rule：已由 2026-05-12 14:39 UTC artifact packet 和 2026-05-13 00:00 UTC forced-failure verifier gate 解锁；后续不再无证据扩写 adapter mapping 或正式设计章节。
...
- 2026-05-13 00:00 UTC：Phase 1a forced-failure verifier slice 已实现；决定把 evidence ref 篡改和 email grounding 篡改纳入 `scripts/validate_repo.py`，要求 verifier 产生 failed report 和 blocking failure，作为进入 Phase 1b 前的机器证据之一。
...
- Phase 1a synthetic fixture artifact packet 已由 `scripts/fixture_runner.py` 生成并由 `scripts/validate_repo.py` 校验；forced-failure validation 已证明 evidence ref 和 email grounding 篡改会触发 failed verifier report。
...
- Phase 1a artifact packet 和 forced-failure verifier gate 通过 PR / GitHub checks 后，Phase 1b 的最小 runner 是否应只接受本地 log 路径和用户目标，还是需要同时引入最小 metadata/run id 输入？
...
### 2026-05-13 00:00 UTC: Phase 1a Forced-Failure Verifier Review

本轮目标：完成最早未完成的 Phase 1a Evidence Review 子切片，证明 verifier 不只会接受正常 artifact packet，也会拒绝被篡改的 evidence refs 和 email grounding。
...
为什么这是下一步：上一轮 artifact packet 已满足 happy path，但计划明确禁止在证明 verifier 能拒绝负例前进入 Phase 1b。forced-failure gate 是进入 Local Read-only Runner 前最小且可机器验证的剩余 Phase 1a 风险。

实现摘要：

- `scripts/fixture_runner.py` 将 `verifier_report.json` 构造逻辑抽为 `verify_artifact_payloads(...)`，供 runner 和验证脚本复用。
- `scripts/validate_repo.py` 在临时生成的 artifact packet 上执行两个 deterministic tamper case：
  - 将 `regression_result.json.evidenceIds` 改成不存在的 id，必须触发 `evidence_refs` blocking failure。
  - 将 `email_draft.md` 中的 structured result id 替换为错误 id，必须触发 `email_draft_uses_structured_facts` blocking failure。
- 正常 5 个 synthetic fixture 的 committed artifact packet 仍与重新生成输出逐文件一致。

验收标准和结果：

- 正常 fixture gate 继续通过：通过。
- evidence ref 篡改会让 verifier report 失败：通过。
- email grounding 篡改会让 verifier report 失败：通过。
- 不引入 SQLite、daemon、capability registry、CUA、IDE、多 agent 或外部副作用：通过。
...
- 本轮证明 verifier 能拒绝 artifact-level tampering，但还没有真实 log 的 parser robustness 证据。
- Phase 1b 仍应保持 read-only、one-shot、本地文件输出，不得直接引入 daemon、SQLite、CUA、IDE 或 multi-agent。

下一轮建议：

```text
执行 Phase 1b Local Read-only Runner 最小切片：
接受一个本地 regression log 路径和用户目标，复用 Phase 1a schema / verifier / artifact writer，输出同一 artifact packet；不引入 daemon、SQLite、正式 capability registry、CUA、IDE 或外部副作用。
```
